...
async fn main() -> Result<()> {
    ...
    if balance_sol < MINIMUM_BALANCE_REQUIRED {
        ...
        error!("   🎯 Mínimo requerido: {:.9} SOL", MINIMUM_BALANCE_REQUIRED);
        error!("   📝 ACCIÓN REQUERIDA:");
        error!("      1. Transferir al menos {:.9} SOL a MainNet wallet", MINIMUM_BALANCE_REQUIRED);
    ...
    if balance_sol < ULTRA_CONSERVATIVE_AMOUNT + 0.005 {
        ...
        error!("   💰 Necesario: {:.9} SOL (operación + fees)", ULTRA_CONSERVATIVE_AMOUNT + 0.005);
    ...
    warn!("   ⏰ Timing: {}ms (probado en DevNet)", TIMING_OPTIMIZATION);
    ...
    if net_change > 0.0 {
        ...
        warn!("⚠️  Loss dentro de límites aceptables: {:.9} SOL", net_change);
    } else {
...
async fn execute_mainnet_arbitrage(client: &RpcClient, wallet: &Keypair) -> Result<()> {
    ...
    info!("   ✅ Wrap completed: {} ({}ms)", wrap_signature, wrap_time.as_millis());
    ...
    info!("   ✅ Unwrap completed: {} ({}ms)", unwrap_signature, unwrap_time.as_millis());

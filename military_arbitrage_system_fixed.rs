...
impl MilitaryArbitrageSystem {
    async fn new() -> Result<Self> {
        ...
        info!(
            "⚔️  Military Arbitrage System (FIXED) loaded: {}",
            wallet_address
        );
        info!(
            "🔬 Monitoring {} critical pools with dynamic parsing",
            monitoring_pools.len()
        );
    ...
    async fn run_direct_arbitrage(&mut self) -> Result<()> {
        ...
        loop {
            ...
            info!(
                "   🎯 Executing opportunity: {:.6}% profit",
    ...
    async fn read_pool_data_direct(&self, pool_address: &str) -> Result<PoolData> {
        ...
        if account.owner.to_string() == RAYDIUM_AMM_PROGRAM {
            ...
            self.parse_orca_whirlpool_dynamic(pool_pubkey, &account)
                .await
        } else {
    ...
    async fn parse_raydium_pool_dynamic(
        &self,
        pool_address: Pubkey,
        account: &Account,
    ) -> Result<PoolData> {
        ...
        Err(anyhow!(
            "Could not parse Raydium pool with any known layout format"
        ))
    }

    // DYNAMIC ORCA POOL PARSING WITH MULTIPLE LAYOUT ATTEMPTS
    async fn parse_orca_pool_dynamic(
        &self,
        pool_address: Pubkey,
        account: &Account,
    ) -> Result<PoolData> {
        ...
        Err(anyhow!(
            "Could not parse Orca pool with any known layout format"
        ))
    }

    // DYNAMIC WHIRLPOOL PARSING
    async fn parse_orca_whirlpool_dynamic(
        &self,
        pool_address: Pubkey,
        account: &Account,
    ) -> Result<PoolData> {
        ...
        if data.len() < 653 {
            return Err(anyhow!(
                "Invalid Orca Whirlpool data length: {}",
                data.len()
            ));
        }
        ...
        Err(anyhow!(
            "Could not parse Whirlpool with any known layout format"
        ))
    }
    ...
    async fn execute_direct_arbitrage(
        &mut self,
        _opportunity: &DirectOpportunity,
    ) -> Result<String> {

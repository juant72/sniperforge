...
async fn main() -> Result<()> {
    ...
    info!("   📈 Profit esperado: {} SOL", params.expected_profit_lamports as f64 / LAMPORTS_PER_SOL as f64);
    info!("   🚨 Pérdida máxima aceptable: {} SOL", params.max_acceptable_loss as f64 / LAMPORTS_PER_SOL as f64);
        ...
        Ok(risk_analysis) => {
            ...
            match risk_analysis.risk_level {
                RiskLevel::Low | RiskLevel::Medium => {
                    ...
                    match execute_risk_calculated_arbitrage(&client, &wallet, &params, &risk_analysis).await {
                        Ok(actual_profit) => {
                            ...
                            info!("🎯 Profit esperado: {:.9} SOL", params.expected_profit_lamports as f64 / LAMPORTS_PER_SOL as f64);
                            info!("📊 Fees estimados: {:.9} SOL", risk_analysis.estimated_fees as f64 / LAMPORTS_PER_SOL as f64);
                            info!("📊 Fees reales: {:.9} SOL", initial_balance - final_balance - actual_profit);
                            
                            if actual_profit > 0.0 {
                                info!("🎉 ¡ARBITRAJE EXITOSO CON GESTIÓN DE RIESGO!");
                                let accuracy = ((actual_profit * LAMPORTS_PER_SOL as f64) / params.expected_profit_lamports as f64) * 100.0;
                                info!("   📊 Precisión predicción: {:.1}%", accuracy);
                            } else {
                                info!("⚠️ Pérdida dentro de rango aceptable: {:.9} SOL", actual_profit);
                            }
                        }
...
async fn perform_comprehensive_risk_analysis(
    ...
    info!("   💰 Rent exempt requerido: {} lamports ({:.9} SOL)", 
    ...
    info!("   💼 Costo total: {} lamports ({:.9} SOL)", 
           total_cost, total_cost as f64 / LAMPORTS_PER_SOL as f64);
    ...
    info!("   🏦 Balance mínimo requerido: {} lamports ({:.9} SOL)", 
           minimum_balance_required, minimum_balance_required as f64 / LAMPORTS_PER_SOL as f64);

    // 5. Verificar balance actual
    let current_balance = client.get_balance(&user_pubkey)?;
    info!("   💳 Balance actual: {} lamports ({:.9} SOL)", 
           current_balance, current_balance as f64 / LAMPORTS_PER_SOL as f64);

    // 6. Calcular profit threshold
    let profit_threshold = total_cost + params.max_acceptable_loss;
    info!("   📊 Profit mínimo requerido: {} lamports ({:.9} SOL)", 
           profit_threshold, profit_threshold as f64 / LAMPORTS_PER_SOL as f64);
...
async fn estimate_transaction_fees(
    ...
    let wrap_instructions = vec![
        system_instruction::transfer(&user_pubkey, &wsol_account, params.amount_lamports + 2_039_280),
...
fn analyze_risk(
    ...
    if expected_profit < profit_threshold {
        return (RiskLevel::High, 
                "Profit esperado menor que fees + pérdida aceptable. Alto riesgo de pérdida neta.".to_string());
    }
...
fn display_risk_analysis(analysis: &RiskAnalysis) {
    ...
    info!("   💸 Fees estimados: {:.9} SOL", analysis.estimated_fees as f64 / LAMPORTS_PER_SOL as f64);
    info!("   🏠 Rent exempt: {:.9} SOL", analysis.rent_exempt_cost as f64 / LAMPORTS_PER_SOL as f64);
    info!("   💼 Costo total: {:.9} SOL", analysis.total_cost as f64 / LAMPORTS_PER_SOL as f64);
    info!("   🏦 Balance mínimo: {:.9} SOL", analysis.minimum_balance_required as f64 / LAMPORTS_PER_SOL as f64);
    info!("   📊 Profit mínimo: {:.9} SOL", analysis.profit_threshold as f64 / LAMPORTS_PER_SOL as f64);
...
async fn execute_risk_calculated_arbitrage(
    ...
    let _wrap_sig = execute_safe_wrap(client, wallet, &wsol_account, params.amount_lamports).await?;

...
async fn analyze_network_conditions(client: &RpcClient) -> Result<()> {
    ...
    if let Some(latest) = recent_performance_samples.first() {
        info!("     ⚡ Samples: {} slots, {} transactions", 
    ...
    info!("     🏠 Rent exempt actual: {} lamports ({:.9} SOL)", 
...
async fn compare_with_successful_conditions(client: &RpcClient) -> Result<()> {
    ...
    info!("     💰 Costo total estimado: {} lamports ({:.9} SOL)", 
...
async fn test_larger_amounts(client: &RpcClient, wallet: &Keypair) -> Result<()> {
    ...
    for amount_sol in test_amounts {
        if balance_sol < amount_sol + 0.01 { // Necesitamos margen para fees
            warn!("     ⚠️ Balance insuficiente para probar {:.3} SOL", amount_sol);
            continue;
        }
        ...
        match execute_large_amount_test(client, wallet, amount_sol).await {
            Ok(_) => {
                ...
                info!("       📊 Resultado {:.3} SOL: {:.9} SOL", amount_sol, change_sol);
                
                if change_sol > 0.0 {
                    info!("       🎉 ¡CANTIDAD GANADORA!: {:.3} SOL genera profit", amount_sol);
...
async fn execute_large_amount_test(
    ...
    let _wrap_sig = execute_clean_wrap(client, wallet, &wsol_account, amount_lamports + rent_exempt).await?;

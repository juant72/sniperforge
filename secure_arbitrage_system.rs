...
impl SecureArbitrageSystem {
            ...
            match self.find_secure_arbitrage_opportunities().await {
                Ok(opportunities) => {
                    if opportunities.is_empty() {
                        ...
                        info!("   🎯 {} SECURE arbitrage opportunities found!", opportunities.len());
                        ...
                        info!("      📊 Slippage: {:.2}%", best_opp.slippage_bps as f64 / 100.0);
                            ...
                            match self.execute_secure_arbitrage(best_opp).await {
                                Ok(signatures) => {
                                    ...
                                    if actual_profit > 0.0 {
                                        ...
                                        return Err(anyhow!("Arbitrage resulted in loss: {:.9} SOL", actual_profit.abs()));
                                    }
                                }
                                Err(e) => {
    ...
    async fn find_secure_arbitrage_opportunities(&self) -> Result<Vec<SecureOpportunity>> {
        ...
        for (input_mint, intermediate_mint, output_mint) in routes {
            if let Some(opportunity) = self.check_secure_arbitrage_route(
                input_mint, 
                intermediate_mint, 
                output_mint, 
                trade_amount
    ...
    async fn check_secure_arbitrage_route(
        ...
        let quote1 = self.get_jupiter_quote_with_slippage(input_mint, intermediate_mint, amount, slippage_bps).await?;
        ...
        if let Some(quote1_data) = quote1 {
            ...
            let quote2 = self.get_jupiter_quote_with_slippage(intermediate_mint, output_mint, intermediate_amount, slippage_bps).await?;
            ...
            if let Some(quote2_data) = quote2 {
                ...
                if final_amount > amount {
                    ...
                    if gross_profit > total_fees {
                        ...
                        info!("         Net profit: {} lamports ({:.4}%)", net_profit, profit_percentage);
    ...
    async fn get_jupiter_quote_with_slippage(&self, input_mint: &str, output_mint: &str, amount: u64, slippage_bps: u16) -> Result<Option<Value>> {
        let url = format!(
    ...
    async fn execute_secure_arbitrage(&self, opportunity: &SecureOpportunity) -> Result<Vec<String>> {
        ...
        if opportunity.amount_in > (balance_before_lamports / 2) {
            return Err(anyhow!("❌ TRADE TOO LARGE: Using more than 50% of balance"));
        }
        ...
        let sig1 = self.execute_secure_jupiter_swap(
        ...
        let actual_intermediate_amount = self.get_actual_token_balance(&opportunity.intermediate_mint).await?;
        
        info!("         💰 Actual intermediate amount received: {} tokens", actual_intermediate_amount);
        ...
        info!("         💰 SOL balance after swap 1: {:.9} SOL", balance_after_swap1);
        info!("         📊 Swap 1 SOL change: {:.9} SOL", swap1_result);
        
        // 🚨 ABORT IF FIRST SWAP LOST TOO MUCH
        if swap1_result < -0.01 { // More than 0.01 SOL loss is suspicious
        ...
        let sig2 = self.execute_secure_jupiter_swap(
            &opportunity.intermediate_mint,
        ...
        if total_profit > 0.0 {
            ...
            error!("      ❌ ARBITRAGE RESULTED IN LOSS: {:.9} SOL", total_profit.abs());
    ...
    async fn get_actual_token_balance(&self, mint: &str) -> Result<u64> {
        ...
        if mint == SOL_MINT {
            ...
            warn!("⚠️ Token balance check not fully implemented for mint: {}", mint);
            Ok(0) // This forces the check to fail safely
        }
    }

    async fn execute_secure_jupiter_swap(&self, input_mint: &str, output_mint: &str, amount: u64, slippage_bps: u16) -> Result<String> {
        // Get fresh quote with proper slippage
        let quote = self.get_jupiter_quote_with_slippage(input_mint, output_mint, amount, slippage_bps).await?

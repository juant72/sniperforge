...
use spl_token::{
    instruction as token_instruction, native_mint,
...
impl RealArbitrageEngine {
    ...
    async fn initialize(&mut self) -> Result<()> {
        ...
        if sol_ui < 0.1 {
            return Err(anyhow::anyhow!(
                "Balance SOL insuficiente. Necesitas al menos 0.1 SOL"
            ));
        }
    ...
    async fn ensure_token_accounts(&mut self) -> Result<()> {
        ...
        if !instructions.is_empty() {
            ...
            info!(
                "📡 Enviando transacción para crear {} cuentas...",
                accounts_to_create.len()
            );
    ...
    async fn execute_real_arbitrage(
        ...
        info!(
            "   💰 Cantidad: {:.6} {}",
            opportunity.amount_in as f64
                / 10_u64.pow(
                    self.config
                        .tokens
                        .get(&opportunity.token_a)
                        .unwrap()
                        .decimals as u32
                ) as f64,
            opportunity.token_a
        ...
        info!(
            "   🔄 Paso 2: {} -> {}",
            opportunity.token_b, opportunity.token_a
        );
        let token_b_balance = self.token_balances.get(&opportunity.token_b).unwrap();
        let amount_b = token_b_balance.balance;

        let signature_2 = self
            .execute_token_swap(&opportunity.token_b, &opportunity.token_a, amount_b)
            .await?;
    ...
    async fn execute_token_swap(
        ...
        if from_token == "SOL" || to_token == "SOL" {
            return self
                .execute_sol_token_swap(from_token, to_token, amount)
                .await;
        }
        ...
        let exchange_rate = self
            .get_simulated_exchange_rate(from_token, to_token)
            .await?;
    ...
    async fn execute_sol_token_swap(
        &self,
        from_token: &str,
        to_token: &str,
        amount: u64,
    ...
    async fn run_arbitrage_session(&mut self, cycles: u32) -> Result<()> {
        info!(
            "🎯 Iniciando sesión de arbitraje real por {} ciclos",
            cycles
        );
    ...
    async fn print_final_report(&mut self) -> Result<()> {
        ...
        info!(
            "   Swaps exitosos: {}",
            self.execution_stats.successful_swaps
        );
        ...
        if !self.execution_stats.execution_times.is_empty() {
            let avg_time = self.execution_stats.execution_times.iter().sum::<u64>()
                / self.execution_stats.execution_times.len() as u64;
...
async fn main() -> Result<()> {
    ...
    let mut arbitrage_engine =
        RealArbitrageEngine::new(config.clone(), rpc_client.clone(), wallet_keypair.clone());

...
impl SimpleArbitrageBot {
    ...
    async fn initialize(&mut self) -> Result<()> {
        ...
        if sol_ui < 0.05 {
            return Err(anyhow::anyhow!("Balance SOL insuficiente. Necesitas al menos 0.05 SOL"));
        }
    ...
    async fn detect_arbitrage_opportunities(&self) -> Result<Vec<ArbitrageOpportunity>> {
        ...
        let available_tokens: Vec<String> = self.token_balances
            .iter()
            .filter(|(symbol, balance)| balance.ui_balance > 1.0 && *symbol != "SOL")
            .map(|(symbol, _)| symbol.clone())
            .collect();

        info!("🔍 Buscando arbitraje entre {} tokens", available_tokens.len());
        ...
        for i in 0..available_tokens.len() {
            for j in i+1..available_tokens.len() {
                ...
                if amount_to_trade >= 1.0 {
                    if let Some(opportunity) = self.simulate_arbitrage_opportunity(
                        token_a.clone(),
                        token_b.clone(),
                        amount_to_trade,
                    ).await? {
        ...
        for (symbol, balance) in &self.token_balances {
            if symbol != "SOL" && balance.ui_balance > 1.0 {
                // SOL -> Token -> SOL
                if let Some(opportunity) = self.simulate_sol_arbitrage_opportunity(
                    symbol.clone(),
                    0.01, // 0.01 SOL
                ).await? {
                    opportunities.push(opportunity);
    ...
    async fn simulate_arbitrage_opportunity(
        ...
        if profit_percentage > 0.2 { // Mínimo 0.2% de profit
            ...
            let amount_out = (amount_a_final * 10_u64.pow(token_a_info.decimals as u32) as f64) as u64;
    ...
    async fn execute_simple_transfer_arbitrage(&mut self, opportunity: &ArbitrageOpportunity) -> Result<String> {
        info!("🚀 Ejecutando arbitraje de transferencia: {}", opportunity.id);
        info!("   📊 {} -> {} -> {}", opportunity.token_from, opportunity.token_to, opportunity.token_from);
        ...
        if opportunity.token_from == "SOL" {
            ...
            self.execute_token_transfer(&opportunity.token_from, 1).await?; // 1 token
        }
        ...
        info!("   ✅ Arbitraje simulado completado en {:.1}ms", execution_time.as_millis());
        info!("   📈 Profit acumulado: {:.6} SOL", 
        ...
        for cycle in 1..=max_cycles {
            ...
            if opportunities.is_empty() {
                ...
                for (i, opportunity) in opportunities.iter().enumerate() {
                    ...
                    info!("   📊 Oportunidad #{}: {} -> {}", 
                          i + 1, opportunity.token_from, opportunity.token_to);
                    info!("      Profit: {:.6} SOL ({:.3}%)", 
                          opportunity.profit_lamports as f64 / LAMPORTS_PER_SOL as f64,
                          opportunity.profit_percentage);
    ...
    async fn print_final_report(&mut self) -> Result<()> {
        ...
        info!("   Trades ejecutados: {}", self.execution_stats.trades_executed);
        info!("   Trades exitosos: {}", self.execution_stats.successful_trades);
        info!("   Profit simulado total: {:.6} SOL", 
              self.execution_stats.total_profit_lamports as f64 / LAMPORTS_PER_SOL as f64);
        
        if self.execution_stats.trades_executed > 0 {
            let success_rate = (self.execution_stats.successful_trades as f64 / self.execution_stats.trades_executed as f64) * 100.0;
...
async fn main() -> Result<()> {
    ...
    // Crear bot
    let mut bot = SimpleArbitrageBot::new(
        config,
        rpc_client,
        wallet_keypair,
    );

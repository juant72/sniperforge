...
async fn main() -> Result<()> {
    ...
    for cycle in 0..10 {
        ...
        let real_opportunities = detect_real_arbitrage_opportunities(
            &http_client,
            &orca_pools,
            &config,
            test_amount,
        ...
        for opportunity in real_opportunities {
            ...
            info!("   Gas estimado: {} lamports", opportunity.estimated_gas_cost);
            
            // Execute if profitable after gas costs
            if opportunity.net_profit > 0.0001 { // Min 0.0001 SOL profit
    ...
    if executed_arbitrages > 0 {
        info!("   Profit promedio: {:.6} SOL", net_profit / executed_arbitrages as f64);
    }
    ...
    match http_client.get(url).send().await {
        Ok(response) => {
            if response.status().is_success() {
                ...
                warn!("   ⚠️ Error obteniendo tokens de Jupiter: {}", response.status());
                Ok(vec![])
            }
        }
        Err(e) => {
...
async fn detect_real_arbitrage_opportunities(
    ...
    if let Some(jupiter_quote) = get_real_jupiter_quote(
        http_client,
        sol_mint,
        usdc_mint,
    ...
    match http_client.post(swap_url).json(&swap_request).send().await {
        Ok(response) => {
            if response.status().is_success() {
                ...
                warn!("     ❌ Error obteniendo swap de Jupiter: {}", response.status());
                return Ok(false);
            }
        }
        Err(e) => {

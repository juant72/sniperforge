...
use crate::config::{EndpointPriority, NetworkConfig, PremiumRpcConfig};
...
impl PremiumRpcManager {
    ...
    pub fn new(network_config: &NetworkConfig) -> Result<Self> {
        ...
        if let Some(premium_config) = &network_config.premium_rpc {
            ...
            for endpoint in &endpoints {
                info!(
                    "   📡 {} (priority: {})",
                    endpoint.provider, endpoint.priority
                );
            }
        }
    ...
    fn build_helius_endpoint(
        ...
        if let (Some(template), Some(api_key)) =
            (&config.helius_rpc_template, api_keys.get("helius"))
        {
    ...
    fn build_ankr_endpoint(
        ...
        if let (Some(template), Some(api_key)) = (&config.ankr_rpc_template, api_keys.get("ankr")) {
            let url = template.replace("{API_KEY}", api_key);
    ...
    fn build_quicknode_endpoint(
        ...
        if let (Some(template), Some(endpoint)) =
            (&config.quicknode_rpc_template, api_keys.get("quicknode"))
        {
    ...
    fn build_alchemy_endpoint(
        ...
        if let (Some(template), Some(api_key)) =
            (&config.alchemy_rpc_template, api_keys.get("alchemy"))
        {
            let url = template.replace("{API_KEY}", api_key);
            let websocket_url = config
                .alchemy_ws_template

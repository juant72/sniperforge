...
impl DexFallbackManager {
    ...
    pub async fn get_quote_with_fallback(
    ...
    ) -> Result<UnifiedQuoteResponse> {
        info!(
            "🔄 Getting quote with fallback: {} {} -> {} {}",
        ...
        for (attempt, dex) in self.fallback_chain.iter().enumerate() {
            ...
            match self.try_get_quote(&request, *dex).await {
                Ok(quote) => {
                    info!(
                        "✅ Quote successful from {}: {} -> {} (price impact: {:.4}%)",
                        dex.as_str(),
                        quote.in_amount,
                        quote.out_amount,
        ...
        Err(last_error
            .unwrap_or_else(|| anyhow!("All DEX providers failed and no fallback enabled")))
    }

    /// Get swap with fallback logic  
    pub async fn build_swap_with_fallback(
        &self,
        request: UnifiedSwapRequest,
    ) -> Result<UnifiedSwapResponse> {
        info!(
            "🔄 Building swap with fallback for DEX: {:?}",
            request.quote.dex_provider
        );

        // Try to use the same DEX that provided the quote first
        match self
            .try_build_swap(&request, request.quote.dex_provider)
            .await
        {
            Ok(swap) => {
                info!(
                    "✅ Swap built successfully with {}",
                    request.quote.dex_provider.as_str()
                );
                return Ok(swap);
            }
            Err(e) => {
        ...
        Err(last_error.unwrap_or_else(|| anyhow!("All DEX swap fallbacks failed")))
    }

    /// Try to get quote from specific DEX
    async fn try_get_quote(
        &self,
        request: &UnifiedQuoteRequest,
        dex: DexProvider,
    ) -> Result<UnifiedQuoteResponse> {
            ...
            DexProvider::Orca => {
                if let Some(orca) = &self.orca_client {
                    ...
                    Ok(UnifiedQuoteResponse {
                        ...
                        route_plan: orca_quote
                            .route
                            .iter()
                            .map(|r| format!("{} ({})", r.pool_id, r.amm_type))
                            .collect(),
    ...
    async fn try_build_swap(
        ...
        match dex {
            ...
            DexProvider::Jupiter => {
                if let Some(jupiter) = &self.jupiter_client {
                    ...
                    let jupiter_swap_request = crate::shared::jupiter_types::SwapRequest {
                        ...
                        prioritizationFeeLamports: request.compute_unit_price_micro_lamports.map(
                            |fee| crate::shared::jupiter_types::PrioritizationFee {
                                priorityLevelWithMaxLamports:
                                    crate::shared::jupiter_types::PriorityLevelConfig {
                                        maxLamports: fee,
            ...
            DexProvider::Raydium => Err(anyhow!("Raydium swap not yet implemented")),
            DexProvider::SplSwap => Err(anyhow!("SPL Token-Swap not yet implemented")),
        }
    }
    ...
    pub async fn get_best_quote(
        ...
        for dex in &self.fallback_chain {
            match self.try_get_quote(&request, *dex).await {
                Ok(quote) => {
                    info!(
                        "💰 Quote from {}: {} out for {} in (impact: {:.4}%)",
                        dex.as_str(),
                        quote.out_amount,
                        quote.in_amount,
                        quote.price_impact_pct
        ...
        info!(
            "🏆 Best quote selected from {}: {} -> {} (impact: {:.4}%)",

...
use crate::shared::alternative_apis::{AlternativeApiManager, BasicConfig, RaydiumPoolInfo};
use crate::shared::helius_websocket::{HeliusPoolCreation, HeliusWebSocketClient};
...
impl PoolDetector {
    ...
    async fn calculate_real_risk_score(&self, pool: &DetectedPool) -> Result<RiskScore> {
        debug!(
            "📊 Calculating REAL risk score for pool: {}",
            &pool.pool_address[..8]
        );

        // Get real liquidity data
        let liquidity_score = if pool.liquidity_usd > 100_000.0 {
            0.9
        } else if pool.liquidity_usd > 50_000.0 {
            0.7
        } else if pool.liquidity_usd > 10_000.0 {
            0.5
        } else {
            0.2
        };
        ...
        let volume_score = if pool.volume_24h > 50_000.0 {
            0.9
        } else if pool.volume_24h > 10_000.0 {
            0.7
        } else if pool.volume_24h > 1_000.0 {
            0.5
        } else {
            0.2
        };
        ...
        let token_age_hours =
            (SystemTime::now().duration_since(UNIX_EPOCH)?.as_secs() - pool.created_at) / 3600;
        let token_age_score = if token_age_hours > 24 {
            0.9
        } else if token_age_hours > 6 {
            0.7
        } else if token_age_hours > 1 {
            0.5
        } else {
            0.2
        };
        ...
        let overall =
            (liquidity_score + volume_score + token_age_score + holder_distribution_score) / 4.0;
    ...
    async fn analyze_pool_opportunity(&mut self, pool: &DetectedPool) -> Result<()> {
        ...
        if pool.risk_score.overall < self.config.min_risk_score {
            debug!(
                "❌ Risk score too low: {:.1}%",
                pool.risk_score.overall * 100.0
            );
            return Ok(());
        }
        ...
        {
            info!(
                "🎯 OPPORTUNITY DETECTED: {} - Expected profit: ${:.2}, Confidence: {:.1}%",
                pool.pool_address,
        ...
        // Trackear el pool
        self.tracked_pools
            .insert(pool.pool_address.clone(), pool.clone());
    ...
    async fn update_tracked_pools(&mut self) -> Result<()> {
        ...
        self.tracked_pools
            .retain(|_, pool| pool.detected_at > cutoff);
    ...
    async fn scan_for_opportunities(&mut self) -> Result<()> {
        ...
        self.opportunities
            .retain(|opp| opp.pool.detected_at > cutoff);
    ...
    async fn scan_for_arbitrage_opportunities(&mut self) -> Result<()> {
        ...
        for pool in pools {
            ...
            {
                if let Ok(jupiter_price_b) =
                    self.get_token_price_from_jupiter(&pool.token_b.mint).await
                {
                    ...
                    if price_diff > 0.02 {
                        ...
                        info!(
                            "💰 Arbitrage opportunity: {:.2}% price difference",
                            price_diff * 100.0
                        );
                        self.opportunities.push(opportunity);
                    }
    ...
    async fn scan_for_price_impact_opportunities(&mut self) -> Result<()> {
        for pool in self.tracked_pools.values() {
            ...
            if pool.price_impact_1k < 1.0 && pool.liquidity_usd > 50000.0 {
                ...
                debug!(
                    "📊 Low price impact opportunity: {:.2}%",
                    pool.price_impact_1k
                );
                self.opportunities.push(opportunity);
            }
        }
    ...
    async fn scan_for_volume_spike_opportunities(&mut self) -> Result<()> {
        for pool in self.tracked_pools.values() {
            ...
            if pool.volume_24h > pool.liquidity_usd * 2.0 && pool.volume_24h > 10000.0 {
                ...
                info!(
                    "📈 Volume spike detected: ${:.0} volume vs ${:.0} liquidity",
    ...
    async fn fetch_real_raydium_pools(&self) -> Result<Vec<DetectedPool>> {
        ...
        match alt_apis.get_comprehensive_pool_data().await {
            Ok(api_pools) => {
                ...
                for (_i, pool) in api_pools.iter().take(10).enumerate() {
                    let detected_pool = DetectedPool {
                        ...
                        risk_score: RiskScore {
                            overall: 0.7,
                            liquidity_score: if pool.liquidity.unwrap_or(0.0) > 100_000.0 {
                                0.8
                            } else {
                                0.4
                            },
                ...
                info!(
                    "✅ Converted {} API pools to DetectedPool format",
                    detected_pools.len()
                );
                Ok(detected_pools)
            }

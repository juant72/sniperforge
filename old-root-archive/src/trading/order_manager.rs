...
impl OrderManager {
    ...
    pub async fn create_stop_loss(&mut self, params: StopLossParams) -> Result<String> {
        ...
        info!(
            "Creating stop-loss order: {} at trigger price {}",
            order_id, params.trigger_price
        );
        ...
        self.price_monitor
            .add_price_watch(&params.token, params.trigger_price)
            .await?;
    ...
    pub async fn create_take_profit(&mut self, params: TakeProfitParams) -> Result<String> {
        ...
        info!(
            "Creating take-profit order: {} at trigger price {}",
            order_id, params.trigger_price
        );
    ...
    pub async fn create_trailing_stop(&mut self, params: TrailingStopParams) -> Result<String> {
        ...
        info!(
            "Creating trailing stop order: {} with trail distance {}",
        ...
        self.active_orders.insert(order_id.clone(), order);
        self.price_monitor
            .add_price_watch(&params.token, initial_stop_price)
            .await?;

        info!(
            "Trailing stop order {} created with initial stop at {}",
            order_id, initial_stop_price
        );
        Ok(order_id)
    }
    ...
    pub async fn monitor_orders(&mut self) -> Result<Vec<ExecutedOrder>> {
        ...
        let order_infos: Vec<(String, String)> = self
            .active_orders
            .iter()
                ...
                match &order_type {
                    OrderType::StopLoss => {
                        if let Some(trigger_price) = trigger_price {
                            if current_price <= trigger_price {
                                info!(
                                    "Stop-loss triggered for order {}: price {} <= trigger {}",
                                ...
                                if let Some(order) = self.active_orders.get(&order_id).cloned() {
                                    match self.execute_order_with_data(order, current_price).await {
                                        ...
                                        Err(e) => {
                                            error!(
                                                "Failed to execute stop-loss order {}: {}",
                                                order_id, e
                                            );
                                            if let Some(order) =
                    ...
                    OrderType::TrailingStop => {
                        if let OrderParameters::TrailingStop(params) = &parameters {
                            ...
                            if should_update_stop {
                                ...
                                info!(
                                    "Updating trailing stop for order {}: {} -> {}",
                                    order_id,
                                    trigger_price.unwrap_or(0.0),
                                    new_stop_price
                                );
                                ...
                                self.price_monitor
                                    .update_price_watch(&token, new_stop_price)
                                    .await?;
                            }
                            ...
                            if let Some(trigger_price) = trigger_price {
                                ...
                                if should_trigger {
                                    ...
                                    {
                                        match self
                                            .execute_order_with_data(order, current_price)
                                            .await
                                        {
                                            Ok(executed) => {
                                                executed_orders.push(executed);
                                            ...
                                            Err(e) => {
                                                error!(
                                                    "Failed to execute trailing stop order {}: {}",
                                                    order_id, e
                                                );
                                                if let Some(order) =
                                                    self.active_orders.get_mut(&order_id)
                    ...
                    OrderType::Conditional => {
                        ...
                        if let Some(order) = self.active_orders.get(&order_id) {
                            if self
                                .check_conditional_trigger_data(&order, current_price)
                                .await?
                            {
                                let order_clone = order.clone();
                                match self
                                    .execute_order_with_data(order_clone, current_price)
                                    .await
                                {
                                    Ok(executed) => {
                                        executed_orders.push(executed);
        ...
        if !executed_orders.is_empty() {
            info!(
                "Executed {} orders in this monitoring cycle",
                executed_orders.len()
            );
        }
    ...
    async fn execute_order_with_data(
        &self,
        mut order: Order,
        current_price: f64,
    ) -> Result<ExecutedOrder> {
    ...
    async fn check_conditional_trigger_data(
        &self,
        _order: &Order,
        _current_price: f64,
    ) -> Result<bool> {
...
impl PriceMonitor {
    ...
    pub async fn update_price_watch(&mut self, token: &str, new_trigger_price: f64) -> Result<()> {
        self.watched_tokens
            .insert(token.to_string(), new_trigger_price);

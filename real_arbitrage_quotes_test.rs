...
async fn main() -> Result<()> {
    ...
    info!("💰 Estrategia: {} SOL -> BONK -> RAY -> SOL", arbitrage_amount);
    ...
    match test_swap_quote(&jupiter, sol_mint, bonk_mint, arbitrage_amount, "SOL", "BONK").await {
        Ok(bonk_amount) => {
            info!("✅ Quote SOL->BONK: {} SOL -> {} BONK", arbitrage_amount, bonk_amount);
            ...
            match test_swap_quote(&jupiter, bonk_mint, ray_mint, bonk_amount, "BONK", "RAY").await {
                Ok(ray_amount) => {
                    info!("✅ Quote BONK->RAY: {} BONK -> {} RAY", bonk_amount, ray_amount);
                    
                    // STEP 3: RAY -> SOL (Test quote)
                    info!("\n🔄 PASO 3: RAY -> SOL (Quote test)");
                    match test_swap_quote(&jupiter, ray_mint, sol_mint, ray_amount, "RAY", "SOL").await {
                        Ok(final_sol) => {
                            ...
                            if profit > 0.001 { // If profit > 0.001 SOL (significant)
                                ...
                                info!("🔧 Jupiter execution falla en DevNet, pero quotes funcionan");
...
async fn test_swap_quote(
    ...
    info!("  🔍 Obteniendo quote: {} {} -> {}", amount, input_symbol, output_symbol);

...
impl StrategyOptimizer {
    ...
    pub async fn optimize_parameters(
            ...
            if generation % 10 == 0 {
                if let Some(best) = &self.best_individual {
                    if let Some(fitness) = &best.fitness {
                        tracing::info!("Generation {}: Best fitness = {:.4}", 
                                     generation, fitness.combined_score);
                    }
                }
            }
        }
        ...
        if let Some(best) = &self.best_individual {
            tracing::info!("Optimization completed in {:.2}s, {} generations", 
                         optimization_time, self.generation);
    ...
    async fn initialize_population(&mut self, initial_parameters: &HashMap<String, f64>) -> Result<()> {
        ...
        if let Some(best_in_generation) = self.population.iter()
            .max_by(|a, b| {
            ...
            if self.best_individual.is_none() || 
               ...
               self.best_individual.as_ref().unwrap().fitness.as_ref().unwrap().combined_score {
    ...
    async fn evaluate_individual(        &self, 
        ...
        if current_index < 14 { return None; } // Need enough data for RSI
    ...
    fn should_exit_position(&self, position: &Position, current_price: f64, parameters: &HashMap<String, f64>) -> bool {
        ...
        match position.trade_type.as_str() {
            "long" => {
                price_change <= -stop_loss || price_change >= *take_profit
            },
            "short" => {
                price_change >= *stop_loss || price_change <= -*take_profit
            },
    ...
    async fn record_generation_stats(&mut self) -> Result<()> {
        ...
        if !fitness_scores.is_empty() {
            let best_fitness = fitness_scores.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
    ...
    fn check_convergence(&self) -> bool {
        ...
        let recent_best: Vec<f64> = self.fitness_history.iter()
            .rev().take(10).map(|gen| gen.best_fitness).collect();
    ...
    fn mutate(&self, individual: &mut Individual) -> Result<()> {
        ...
        for (key, value) in &mut individual.parameters {
            if rng.gen::<f64>() < 0.3 { // 30% chance to mutate each parameter
    ...
    pub fn get_optimization_stats(&self) -> OptimizationStats {
        OptimizationStats {
            ...
            best_fitness: self.best_individual.as_ref()
                .and_then(|ind| ind.fitness.as_ref())
                .map(|f| f.combined_score),
            convergence_progress: self.fitness_history.len() as f64 / self.config.generations as f64,
            last_optimization: self.last_optimization,
        }
    }

    /// Update strategy optimizer configuration
    pub async fn update_configuration(&mut self, new_config: StrategyOptimizerConfig) -> Result<()> {

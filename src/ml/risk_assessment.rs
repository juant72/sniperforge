...
use super::{FeatureVector, MLPrediction, RiskAssessmentConfig};
...
impl RiskAssessor {
    ...
    async fn assess_volatility_risk(&mut self, features: &FeatureVector) -> Result<f64> {
        ...
        let historical_vol = features
            .features
            .get("historical_volatility")
            .unwrap_or(&0.0);
    ...
    fn generate_risk_recommendations(
        ...
        match risk_level {
            RiskLevel::Low => {
                recommendations
                    .push("Risk level is low. Consider normal position sizing.".to_string());
                recommendations.push("Monitor for regime changes.".to_string());
            }
            RiskLevel::Medium => {
                recommendations
                    .push("Moderate risk detected. Use conservative position sizing.".to_string());
                recommendations.push("Implement tight stop-losses.".to_string());
            }
            RiskLevel::High => {
                recommendations.push(
                    "High risk environment. Reduce position sizes significantly.".to_string(),
                );
                recommendations
                    .push("Consider avoiding new positions until risk subsides.".to_string());
    ...
    pub async fn update_risk_models(&mut self, training_data: &[FeatureVector]) -> Result<()> {
        ...
        tracing::info!(
            "Risk assessment models updated with {} samples",
            training_data.len()
        );
        Ok(())
    }

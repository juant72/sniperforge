...
impl TransactionMonitor {
    ...
    pub async fn monitor_transaction(&mut self, signature: &str) -> Result<TransactionResult> {
        ...
        self.pending_transactions
            .insert(signature.to_string(), start_time);
            ...
            match self.check_transaction_status(signature).await {
                ...
                Err(e) => {
                    if attempt >= self.config.retry_attempts {
                        error!(
                            "❌ Failed to check transaction after {} attempts: {}",
                            attempt, e
                        );
                        return Err(e);
                    }
    ...
    async fn get_transaction_details(&self, signature: &Signature) -> Result<TransactionResult> {
            ...
            .get_transaction_with_config(signature, config)
        {
            Ok(transaction) => {
                ...
                let fee_paid = transaction.transaction.meta.as_ref().map(|meta| meta.fee);
    ...
    pub async fn monitor_multiple_transactions(
        ...
        info!(
            "🔍 Monitoring {} transactions concurrently",
            signatures.len()
        );
...
pub async fn test_transaction_monitoring() -> Result<()> {
    ...
    let monitor = TransactionMonitor::new("https://api.devnet.solana.com", config);

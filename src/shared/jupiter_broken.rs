...
impl JupiterClient {
    ...
    impl Jupiter {
        ...
        /// Execute complete swap with wallet integration
        pub async fn execute_swap_with_wallet(
            &self,
            quote: &QuoteResponse,
            wallet_manager: &crate::shared::wallet_manager::WalletManager,
            wallet_name: &str,
            ...
            info!("🚀 Starting complete swap execution with wallet integration...");

            // Step 1: Get wallet address
            let wallet_address = wallet_manager.get_wallet_address(wallet_name).await?;
            info!("💳 Using wallet: {} ({})", wallet_name, wallet_address);

            // Step 2: Build the swap transaction
            let swap_response = self.build_swap_transaction_internal(quote, &wallet_address).await?;
            info!("🔧 Swap transaction built successfully");

            // Step 3: Deserialize the transaction from base64
            ...
            // Step 4: Get recent blockhash and update transaction
            ...
            let signed_transaction = wallet_manager
                .sign_transaction(wallet_name, transaction, amount_sol, description)
                .await?;

            info!("✅ Transaction signed successfully");

            // Step 6: Simulate the transaction first (safety check)
            ...
            // Step 7: Send the transaction to the blockchain
            ...
            // Step 8: Verify the transaction status
            ...
            if success {
                ...
                Err(anyhow!("Transaction failed on blockchain"))
            }
        }
        ...
        async fn build_swap_transaction_internal(
            ...
            let swap_url = format!(
                "{}/v6/swap",
                self.client.base_url.replace("lite-api.jup.ag", "quote-api.jup.ag")
            );
            ...
            info!("✅ Swap transaction built successfully");
            debug!(
                "Transaction base64 length: {}",
                swap_response.swapTransaction.len()
            );
            debug!(
                "Last valid block height: {}",
                swap_response.lastValidBlockHeight
            );

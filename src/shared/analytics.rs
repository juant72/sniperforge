...
use crate::shared::pool_detector::{DetectedPool, TradingOpportunity, OpportunityType, RiskScore};
...
impl PoolAnalyticsEngine {
    ...
    pub fn add_pools(&mut self, pools: Vec<DetectedPool>) -> Result<()> {
        if let Some(session) = &mut self.current_session {
            ...
            info!("📈 Added {} pools to analytics", session.pools_detected.len());
        }
        Ok(())
    }
    ...
    pub fn add_opportunities(&mut self, opportunities: Vec<TradingOpportunity>) -> Result<()> {
        if let Some(session) = &mut self.current_session {
            ...
            info!("🎯 Added {} opportunities to analytics", session.opportunities_found.len());
        }
        Ok(())
    }
    ...
    fn analyze_patterns(&self, session: &PoolAnalytics) -> Result<PatternAnalysis> {
        info!("🔍 Analyzing patterns in {} pools and {} opportunities...", 
              session.pools_detected.len(), session.opportunities_found.len());

        let liquidity_distribution = self.analyze_liquidity_distribution(&session.pools_detected)?;
        let token_patterns = self.analyze_token_patterns(&session.pools_detected)?;
        let opportunity_patterns = self.analyze_opportunity_patterns(&session.opportunities_found)?;
        let time_patterns = self.analyze_time_patterns(&session.pools_detected, &session.opportunities_found)?;
    ...
    fn analyze_liquidity_distribution(&self, pools: &[DetectedPool]) -> Result<LiquidityDistribution> {
        if pools.is_empty() {
        ...
        let median_liquidity_usd = if liquidity_values.len() % 2 == 0 {
            (liquidity_values[liquidity_values.len() / 2 - 1] + liquidity_values[liquidity_values.len() / 2]) / 2.0
        } else {
    ...
    fn analyze_token_patterns(&self, pools: &[DetectedPool]) -> Result<TokenPatterns> {
        ...
        for pool in pools {
            ...
            *token_risk_distribution.entry(risk_level.to_string()).or_insert(0) += 1;
        }
        ...
        let new_token_rate = if pools.is_empty() { 0.0 } else {
            // Estimate based on unique tokens found
            let unique_tokens: std::collections::HashSet<String> = pools.iter()
    ...
    fn update_token_activity(&self, activity_map: &mut HashMap<String, TokenActivity>, token: &crate::shared::pool_detector::TokenInfo, pool: &DetectedPool) {
        let entry = activity_map.entry(token.symbol.clone()).or_insert(TokenActivity {
        ...
        entry.average_risk_score = (entry.average_risk_score * (entry.pool_count - 1) as f64 + pool.risk_score.overall) / entry.pool_count as f64;
    }

    /// Analyze opportunity patterns
    fn analyze_opportunity_patterns(&self, opportunities: &[TradingOpportunity]) -> Result<OpportunityPatterns> {
        ...
        for opp in opportunities {
            *opportunity_distribution.entry(opp.opportunity_type.clone()).or_insert(0) += 1;
        ...
        let profitability_analysis = ProfitabilityAnalysis {
            ...
            profit_ranges,
            highest_profit_opportunities: opportunities.iter()
    ...
    fn analyze_time_patterns(&self, pools: &[DetectedPool], opportunities: &[TradingOpportunity]) -> Result<TimePatterns> {
        ...
        if !pools.is_empty() {
            let snapshot = ActivitySnapshot {
                ...
                average_risk_score: pools.iter().map(|p| p.risk_score.overall).sum::<f64>() / pools.len() as f64,
            };
    ...
    fn analyze_risk_patterns(&self, pools: &[DetectedPool]) -> Result<RiskPatterns> {
        ...
        for pool in pools {
            ...
            *overall_risk_distribution.entry(risk_level.to_string()).or_insert(0) += 1;

            // Count rug indicators
            for indicator in &pool.risk_score.rug_indicators {
                *rug_indicators_frequency.entry(indicator.clone()).or_insert(0) += 1;
            }
        }
    ...
    fn calculate_performance_metrics(&self, session: &PoolAnalytics) -> Result<PerformanceMetrics> {
        let system_efficiency = SystemEfficiency {
            ...
            opportunities_per_scan: session.opportunities_found.len() as f64 / session.total_scans.max(1) as f64,
...
"#,
            ...
            session.pattern_analysis.liquidity_distribution.total_liquidity_usd,
            session.pattern_analysis.liquidity_distribution.average_liquidity_usd,
            session.pattern_analysis.liquidity_distribution.median_liquidity_usd,
            session.pattern_analysis.liquidity_distribution.high_liquidity_pools.len(),
            session.pattern_analysis.opportunity_patterns.profitability_analysis.total_potential_profit_usd,
            session.pattern_analysis.opportunity_patterns.profitability_analysis.average_profit_per_opportunity,
            session.pattern_analysis.opportunity_patterns.confidence_distribution.get("high").unwrap_or(&0),
            session.opportunities_found.first().map(|o| &o.opportunity_type).unwrap_or(&OpportunityType::NewPoolSnipe),
            session.performance_metrics.system_efficiency.scans_per_second,
            session.performance_metrics.detection_latency_ms,
            session.performance_metrics.system_efficiency.resource_utilization * 100.0,
            session.pattern_analysis.risk_patterns.overall_risk_distribution.get("low").unwrap_or(&0),
            session.pattern_analysis.risk_patterns.overall_risk_distribution.get("medium").unwrap_or(&0),
            session.pattern_analysis.risk_patterns.overall_risk_distribution.get("high").unwrap_or(&0),
            session.pools_detected.iter().map(|p| p.risk_score.overall).sum::<f64>() / session.pools_detected.len().max(1) as f64,
            session.pattern_analysis.time_patterns.pool_creation_rate,
            session.pattern_analysis.token_patterns.token_pair_frequency.keys().take(3).cloned().collect::<Vec<_>>().join(", "),
            session.pattern_analysis.opportunity_patterns.timing_analysis.peak_activity_periods.join(", "),
        )
    }
}

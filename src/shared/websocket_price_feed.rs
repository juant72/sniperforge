                        ...
                        match msg {
                            Ok(Message::Text(_)) => {
                                ...
                                if activity_counter % 3 == 0
                                    || last_triggered_update.elapsed() > Duration::from_secs(8)
                                {
                                    ...
                                    if let Some(&token) =
                                        tokens.get(activity_counter % tokens.len())
                                    {
                                        if let Ok(price) = Self::fetch_jupiter_price(token).await {
                                            ...
                                            *last_update_pool.write().await = Instant::now();
                                            ...
                                            debug!(
                                                "⚡ Pool activity triggered update: {} = ${:.6}",
                                                token, price
                                            );
                                        }
                                    }
    ...
    pub async fn get_price_realtime(&self, token_mint: &str) -> Option<f64> {
        ...
        if let Some(price_data) = cache.get(token_mint) {
            ...
            if price_data.timestamp.elapsed() < Duration::from_secs(60) {
                debug!(
                    "⚡ WebSocket cache hit: {} = ${:.6} (age: {:?})",
                    token_mint,
                    price_data.price,
                    price_data.timestamp.elapsed()
                );
                return Some(price_data.price);
            } else {
                debug!(
                    "⏰ Cached price too old for {}: {:?}",
                    token_mint,
                    price_data.timestamp.elapsed()
                );
            }
    ...
    pub async fn get_all_cached_prices(&self) -> HashMap<String, (f64, Duration, String)> {
        ...
        cache
            ...
            .map(|(token, data)| {
                (
                    token.clone(),
                    (data.price, data.timestamp.elapsed(), data.source.clone()),
                )
            })
            .collect()
    }

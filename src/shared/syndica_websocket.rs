...
impl SyndicaConfig {
    ...
    pub fn devnet() -> Self {
        Self {
            ...
            endpoint: "wss://api.devnet.solana.com".to_string(),
            reconnect_attempts: 5,
            ping_interval: Duration::from_secs(30),
        }
    }

    /// Create configuration from network config
    pub fn from_network_config(network_config: &crate::config::NetworkConfig) -> Self {
        match network_config.environment.as_str() {
            "mainnet" => Self::mainnet(),
            "devnet" => Self::devnet(),
            _ => {
                // Use the configured websocket URL or fallback to devnet
                Self {
                    access_token: std::env::var("SYNDICA_TOKEN")
                        .unwrap_or_else(|_| "test-token".to_string()),
                    endpoint: network_config.websocket_url().to_string(),
                    reconnect_attempts: 5,
                    ping_interval: Duration::from_secs(30),
                }
            }
...
impl SyndicaWebSocketClient {
    ...
    pub async fn start_price_simulation(&self) -> Result<()> {
        ...
        Err(anyhow::anyhow!(
            "Price simulation not allowed - real data only"
        ))
    }
    ...
    pub async fn connect(&mut self) -> Result<()> {
        ...
        let url = format!(
            "{}/api-key/{}",
            self.config.endpoint, self.config.access_token
        );
    ...
    pub async fn get_price_ultra_fast(&self, token_mint: &str) -> Result<Option<f64>> {
        ...
        if let Some(entry) = cache.get(token_mint) {
            ...
            debug!(
                "🕐 Found price ${:.4}, age: {}ms, source: {:?}, confidence: {:?}",
                entry.price,
                age.as_millis(),
                entry.source,
                entry.confidence
            ...
            match entry.source {
                PriceSource::SyndicaRealtime => {
                    ...
                    if age < Duration::from_millis(50) {
                        ...
                        debug!(
                            "⚠️ Aged real data with discount: ${:.4} -> ${:.4}",
    ...
    pub async fn get_price_with_metadata(
        &self,
        token_mint: &str,
    ) -> Result<Option<(f64, Duration, bool, String)>> {
        let start = Instant::now();
        let cache = self.price_cache.read().await;
                        ...
                        if let Some(owner) = info.get("owner").and_then(|v| v.as_str()) {
                            ...
                            match owner {
                                "675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8" => {
                                    ...
                                    return Self::calculate_price_from_raydium_account(
                                        mint,
                                        token_amount,
                                        info,
                                    )
                                ...
                                "9W959DqEETiGZocYWCQPaJ6sBmUzgfxXfqGeTEdp3aQP" => {
                                    ...
                                    return Self::calculate_price_from_orca_account(
                                        mint,
                                        token_amount,
                                        info,
                                    )
                    ...
                    if let Some(data_array) = data.as_array() {
                        ...
                        if data_array.len() >= 2 {
                            if let (Some(data_base64), Some(encoding)) =
                                (data_array[0].as_str(), data_array[1].as_str())
                            {
                                if encoding == "base64" {
    ...
    pub async fn get_price_ultra_safe(&self, token_mint: &str) -> Result<Option<f64>> {
        ...
        if let Some(entry) = cache.get(token_mint) {
            ...
            {
                info!(
                    "✅ ULTRA-FRESH real-time data: {} = ${:.4} ({}ms old)",
                    token_mint,
                    entry.price,
                    age.as_millis()
                );
                ...
                warn!(
                    "⚠️ Data not ultra-fresh enough: {}ms old, source: {:?}",
                    age.as_millis(),
                    entry.source
                );
            }
    ...
    pub async fn get_latest_websocket_price(&self, token_mint: &str) -> Result<Option<f64>> {
        ...
        if let Some(entry) = cache.get(token_mint) {
            ...
            if age < Duration::from_millis(100) && entry.source == PriceSource::SyndicaRealtime {
                debug!(
                    "⚡ Fresh WebSocket price found: {} ({}ms old)",
                    entry.price,
                    age.as_millis()
                );
                return Ok(Some(entry.price));
                ...
                debug!(
                    "⚠️ Data too old for cache-free operation: {}ms",
                    age.as_millis()
                );
            }
        }
    ...
    async fn calculate_price_from_orca_account(
        ...
        debug!(
            "📊 Orca account update: {} amount changed to {}",
            mint, amount
        );
        None
    }
    ...
    async fn parse_raydium_amm_data(pubkey: &str, data_base64: &str) -> Option<SyndicaPriceUpdate> {
        use base64::Engine;
            ...
            if data_bytes.len() >= 64 {
                ...
                if data_bytes.len() >= 48 {
                    ...
                    if base_reserve_bytes.len() == 8 && quote_reserve_bytes.len() == 8 {
                        let base_reserve =
                            u64::from_le_bytes(base_reserve_bytes.try_into().unwrap());
                        let quote_reserve =
                            u64::from_le_bytes(quote_reserve_bytes.try_into().unwrap());

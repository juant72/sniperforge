...
use crate::shared::wallet_manager::{RiskManagement, WalletConfig, WalletManager, WalletType};
...
impl MainNetTradingEngine {
    pub async fn new(
        ...
        println!(
            "💰 Max Capital: ${}, Max Trade: ${}, Daily Limit: ${}",
            config.max_capital_usd, config.max_single_trade_usd, config.daily_limit_usd
        ); // Initialize trade executor for real MainNet trading
        let trade_executor =
            Arc::new(TradeExecutor::new(platform_config.clone(), TradingMode::MainNet).await?); // Initialize pool detector for opportunity detection
    ...
    async fn verify_wallet_setup(&self) -> Result<()> {
        ...
        if let Some(balance) = self.wallet_manager.get_wallet_balance("trading").await {
            ...
            if balance > self.config.max_sol_balance {
                return Err(PlatformError::Trading(format!(
                    "Trading wallet balance ({} SOL) exceeds maximum allowed ({} SOL)",
            ...
            return Err(PlatformError::Wallet("Trading wallet not found".to_string()).into());
        }
    ...
    pub async fn execute_trade(
        ...
        if expected_profit_percent < self.config.min_profit_threshold_percent {
            println!(
                "❌ Trade rejected: Profit {} < minimum threshold {}%",
        ...
        if self.positions.read().await.len() >= self.config.max_positions as usize {
            println!(
                "❌ Trade rejected: Maximum positions ({}) reached",
        ...
        // Execute the trade
        println!(
            "🔥 Executing REAL MainNet trade: {} ${:.2}",
            symbol, amount_usd
        );
        ...
        println!(
            "✅ Real trade executed: {} (${:.2})",
            position_id, amount_usd
        );
        println!(
            "   Entry: ${:.4}, Stop Loss: ${:.4}, Target: ${:.4}",
    ...
    async fn validate_trade_parameters(&self, amount_usd: f64) -> Result<()> {
        // Check single trade limit
        if amount_usd > self.config.max_single_trade_usd {
            return Err(PlatformError::Trading(format!(
                "Trade amount ${:.2} exceeds single trade limit ${:.2}",
        ...
        if daily_volume + amount_usd > self.config.daily_limit_usd {
            return Err(PlatformError::Trading(format!(
                "Trade would exceed daily limit: ${:.2} + ${:.2} > ${:.2}",
        ...
        if total_exposure + amount_usd > self.config.max_capital_usd {
            return Err(PlatformError::Trading(format!(
                "Trade would exceed capital limit: ${:.2} + ${:.2} > ${:.2}",
...

                {
                    ...
                    for (id, position) in positions_guard.iter_mut() {
                        ...
                        position.unrealized_pnl =
                            (price - position.entry_price) * position.amount_sol;
    ...
    async fn start_price_monitoring(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                match blockchain_engine
                    .get_real_time_price("So11111111111111111111111111111111111111112")
                    .await
                {
                    Ok(price_data) => {
    ...
    pub async fn emergency_stop(&self, reason: &str) -> Result<()> {
        ...
        *self.emergency_stopped.write().await = true;
        ...
        // Lock all wallets
        self.wallet_manager
            .emergency_stop(reason.to_string())
            .await?;
    ...
    pub async fn get_health_status(&self) -> HealthStatus {
        ...
        if *self.emergency_stopped.read().await {
            ...
            metrics.insert(
                "daily_volume".to_string(),
                serde_json::json!(stats.daily_volume_usd),
            );
            metrics.insert(
                "daily_limit".to_string(),
                serde_json::json!(self.config.daily_limit_usd),
            );
    ...
    async fn start_opportunity_detection(&self) {
        let pool_detector = Arc::clone(&self.pool_detector);
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                match Self::detect_opportunities(&pool_detector).await {
                    Ok(opportunities) => {
                        println!(
                            "🔍 Scanned for opportunities - found: {}",
                            opportunities.len()
                        );
                        ...
                        if !opportunities.is_empty() {
                            ...
                            for opportunity in opportunities {
                                ...
                                if Self::validate_opportunity(&opportunity, &config).await {
                                    println!(
                                        "✅ Opportunity validated: {} - Expected profit: ${:.2}",
                                        opportunity.pool.pool_address,
                                        opportunity.expected_profit_usd
                                    );
                            ...
                            if interval_count % 12 == 0 {
                                // Every 60 seconds (12 * 5s)
                                println!(
                                    "🔍 Scanning MainNet pools... (no opportunities found yet)"
                                );
                            }
                        }
                    }
    ...
    async fn detect_opportunities(
        pool_detector: &Arc<Mutex<PoolDetector>>,
    ...
    async fn execute_opportunity(
        ...
        let trade_request = TradeRequest {
            ...
            amount_in: (opportunity
                .recommended_size_usd
                .min(config.max_single_trade_usd)
                / 150.0
                * 1_000_000_000.0) as u64, // Convert USD to lamports
        ...
        match trade_executor.execute_trade(trade_request).await {
            Ok(result) => {
                if result.success {
                    println!(
                        "✅ REAL TRADE EXECUTED: {}",
                        result.transaction_signature.unwrap_or("NO_TX".to_string())
                    );
                    Ok(true)
                } else {
                    println!(

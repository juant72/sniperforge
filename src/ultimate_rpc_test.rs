...
impl UltimateRpcComparison {
        ...
        if std::env::var("SYNDICA_TOKEN").is_ok() {
            ...
            match Self::create_syndica_client().await? {
                Some(syndica) => {
                    ...
                    let syndica_results = self
                        .test_syndica_websocket(&syndica, test_token, iterations)
                        .await?;
    ...
    async fn test_syndica_websocket(
        &self,
        syndica: &SyndicaWebSocketClient,
        token: &str,
        iterations: usize,
    ) -> Result<Vec<Duration>> {
        let mut results = Vec::new();
        ...
        if !successful_results.is_empty() {
            let avg_ms = successful_results
                .iter()
                .map(|d| d.as_millis())
                .sum::<u128>()
                / successful_results.len() as u128;
            let avg_micros = successful_results
                .iter()
                .map(|d| d.as_micros())
                .sum::<u128>()
                / successful_results.len() as u128;
            let min_micros = successful_results
                .iter()
                .map(|d| d.as_micros())
                .min()
                .unwrap_or(0);
            let max_micros = successful_results
                .iter()
                .map(|d| d.as_micros())
                .max()
                .unwrap_or(0);
            ...
            if cache_health.total_entries == 0 {
                ...
                println!(
                    "   ✅ Data Source: WebSocket cache ({} entries)",
            ...
            if prices_seen.len() >= 2 {
                ...
                println!(
                    "   Price Range: ${:.4} - ${:.4} (±{:.2}%)",
                    min_price, max_price, price_variation
                );
            }
        } else {
            ...
            if !price_found {
                match self.jupiter_client.get_price(token).await {
                    Ok(Some(price)) => {
                        ...
                        results.push(duration);
                        debug!(
                            "Hybrid {}: ${:.4} via HTTP fallback in {}ms",
                            i,
                            price,
                            duration.as_millis()
                        );
    ...
    fn compare_results(
        ...
        if avg2 == 0 && avg1 == 0 {
            ...
            println!(
                "   🚀 {} has sub-millisecond latency! ({}ms saved)",
                name2, avg1
            );
        } else if avg1 == 0 {
            println!(
    ...
    fn show_recommendations(&self, http_results: &[Duration], hybrid_results: &[Duration]) {
        ...
        let http_avg =
            http_results.iter().map(|d| d.as_millis()).sum::<u128>() / http_results.len() as u128;
        let hybrid_avg = hybrid_results.iter().map(|d| d.as_millis()).sum::<u128>()
            / hybrid_results.len() as u128;

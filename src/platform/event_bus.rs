...
impl EventBus {
    ...
    pub async fn subscribe(
        ...
        info!("Registered event subscriber: {} ({})", subscriber_name, subscriber_id);
        Ok((subscriber_id, rx))
    }
    ...
    pub async fn unsubscribe(&self, subscriber_id: Uuid) -> Result<()> {
        ...
        if let Some(subscriber) = subscribers.remove(&subscriber_id) {
            info!("Unregistered event subscriber: {} ({})", subscriber.name, subscriber_id);
            Ok(())
        } else {
        ...
        for subscriber in subscribers.values() {
            ...
            if self.subscriber_interested(&subscriber.event_types, &event.event_type) {
                match subscriber.sender.send(event.clone()) {
                    ...
                    Err(e) => {
                        warn!("Failed to deliver event to subscriber {}: {}", subscriber.name, e);
                    }
                }
            }
        }
    ...
    pub async fn get_events_by_type(&self, event_type: EventType, limit: Option<usize>) -> Vec<PlatformEvent> {
        let buffer = self.event_buffer.read().await;
    ...
    fn subscriber_interested(&self, subscriber_types: &[EventType], event_type: &EventType) -> bool {

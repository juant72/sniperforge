...
impl SniperForgePlatform {
    ...
    pub async fn shutdown(&self) -> Result<()> {
        ...
        *self.is_running.write().await = false;
        ...
        if let Err(e) = self.event_bus.publish_simple(
            event_bus::EventType::SystemMetrics,
            "platform".to_string(),
            serde_json::json!({
    ...
    pub async fn get_metrics(&self) -> Result<PlatformMetrics> {
        ...
        Ok(PlatformMetrics {
            ...
            active_bots: bot_metrics.iter().filter(|m| {
                matches!(m.1, crate::types::BotStatus::Running)
            }).count(),
            total_trades: bot_metrics.iter().map(|(metrics, _)| metrics.operations_count).sum(),
            ...
            avg_latency_ms: if !bot_metrics.is_empty() {
                bot_metrics.iter().map(|(metrics, _)| metrics.avg_latency_ms).sum::<f64>() / bot_metrics.len() as f64
            } else {
                0.0
            },
    ...
    async fn perform_health_check(&self) -> Result<()> {
        ...
        if !shared_health.is_healthy {
            warn!("🔧 Shared services health check failed: {:?}", shared_health.message);
        }
        
        // Check bot manager health
        let bot_health = self.bot_manager.health_check().await?;
        if !bot_health.is_healthy {
            warn!("🤖 Bot manager health check failed: {:?}", bot_health.message);
        }
        ...
        if !resource_health.is_healthy {
            warn!("⚖️ Resource coordinator health check failed: {:?}", resource_health.message);
        }
    ...
    fn parse_bot_type(&self, bot_type_str: &str) -> Option<BotType> {
        match bot_type_str.to_lowercase().as_str() {
            ...
            "dca" | "dollar-cost-average" | "dollar_cost_average" => Some(BotType::DollarCostAverage),            _ => None,
        }
    }

...
impl RealArbitrageHunter {
    async fn new() -> Result<Self> {
        ...
        info!("✅ Real Arbitrage Hunter loaded: {} (from: {})", wallet_address, used_path);
            ...
            match self.scan_real_arbitrage_opportunities().await {
                Ok(opportunities) => {
                    if opportunities.is_empty() {
                        ...
                        info!("   🎯 {} REAL arbitrage opportunities detected!", opportunities.len());
                        ...
                        for (i, opp) in sorted_opps.iter().take(5).enumerate() {
                            ...
                            info!("   {} {}: {} → {} ({:.9} SOL, {:.2}%)", 
                                  i + 1, status, opp.buy_dex, opp.sell_dex, 
                                  opp.expected_profit, opp.profit_percentage);
                        }
                    }
                }
                Err(e) => {
                ...
                match self.get_current_performance().await {
                    Ok((current_balance, net_profit)) => {
                        ...
                        if successful_trades > 0 {
                            info!("   📊 Average profit per trade: {:.9} SOL", total_profit / successful_trades as f64);
                        }
                        info!("   💵 USD equivalent: ${:.2}", net_profit * 170.0);
                        
                        if net_profit > 0.0 {
                            info!("   🎉 ESTADO: GANANDO DINERO! +{:.2}%", (net_profit / self.initial_balance) * 100.0);
                        } else {
    ...
    async fn scan_real_arbitrage_opportunities(&self) -> Result<Vec<RealArbitrageOpportunity>> {
        ...
        for i in 0..prices.len() {
            for j in i+1..prices.len() {
                ...
                if profit_percentage > 0.5 { // 0.5% minimum difference
                    ...
                    let expected_profit_usd = (sell_price - buy_price) * (trade_amount_usd / buy_price) * 0.7; // 70% efficiency
            ...
            match self.http_client.get(url).send().await {
                Ok(response) => {
                    if response.status().is_success() {
                        ...
                        warn!("   ⚠️ CoinGecko API returned status {} (attempt {})", response.status(), attempt);
                    }
                }
                Err(e) => {
                    warn!("   ⚠️ CoinGecko API request failed (attempt {}): {}", attempt, e);
                }
            }
    ...
    async fn execute_real_arbitrage(&self, opportunity: &RealArbitrageOpportunity) -> Result<f64> {
        ...
        info!("      💰 Comprar en {}: ${:.6}", opportunity.buy_dex, opportunity.buy_price);
        info!("      💰 Vender en {}: ${:.6}", opportunity.sell_dex, opportunity.sell_price);
        info!("      📈 Profit esperado: {:.9} SOL", opportunity.expected_profit);

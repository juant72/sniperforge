...
impl ArbitrageAnalyzer {
    ...
    async fn analyze_current_market(&self) -> Result<()> {
        ...
        let test_pairs = vec![
            ...
            ("SOL/USDC", 0.001, "So11111111111111111111111111111111111111112", "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"),
            ...
            ("USDC/USDT", 50.0, "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v", "Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB"),
            ("USDC/USDT", 100.0, "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v", "Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB"),
        ...
        for (pair_name, amount, mint_a, mint_b) in test_pairs {
            ...
            match self.test_arbitrage_opportunity(mint_a, mint_b, amount, pair_name).await {
                Ok(Some((profit, route1_price, route2_price, slippage))) => {
                    let profit_multiple = profit / fee_cost;
                    let status = if profit > fee_cost * 3.0 { "🚀 GOOD" } else if profit > 0.0 { "💡 SMALL" } else { "❌ LOSS" };
                    
                    info!("   {} Profit: {:.9} SOL ({:.1}x fees)", status, profit, profit_multiple);
                    info!("   📈 Route prices: {:.6} → {:.6}", route1_price, route2_price);
        ...
        info!("   💰 Profitable opportunities: {}", profitable_opportunities);
        info!("   📈 Success rate: {:.1}%", (profitable_opportunities as f64 / total_tested as f64) * 100.0);
        ...
        if profitable_opportunities == 0 {
            ...
            info!("\n✅ Found {} opportunities! Consider running the hunter.", profitable_opportunities);
        }
    ...
    async fn test_arbitrage_opportunity(
        ...
        if let Ok(Some(route_1_data)) = self.get_jupiter_quote(mint_a, mint_b, amount_units).await {
            ...
            if intermediate_amount > 0 {
                // Route 2: B -> A
                if let Ok(Some(route_2_data)) = self.get_jupiter_quote(mint_b, mint_a, intermediate_amount).await {
                    ...
                    match response.json::<Value>().await {
                        Ok(data) => {
                            if data.get("error").is_some() {
                                ...
                                return self.get_jupiter_quote_fallback(input_mint, output_mint, amount).await;
                            } else {
                                Ok(Some(data))
                            }
                        }
    ...
    async fn get_jupiter_quote_fallback(&self, input_mint: &str, output_mint: &str, amount: u64) -> Result<Option<Value>> {
        let url = format!(

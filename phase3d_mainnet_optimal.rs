...
async fn main() -> Result<()> {
    ...
    match execute_optimal_scale_2c(&client, &wallet).await {
        Ok(_) => {
            ...
            info!("\n📊 === COMPREHENSIVE ANALYSIS ===");
            info!(
                "   🌐 DevNet actual: +{:.9} SOL on 0.01 SOL",
                devnet_reference
            );
            info!(
                "   📈 Expected scaled: +{:.9} SOL on 0.03 SOL",
                expected_profit
            );
            info!(
                "   🌍 MainNet actual: {:+.9} SOL on 0.03 SOL",
                actual_profit
            );
            ...
            info!(
                "   ⚡ MainNet efficiency: {:.2}% of DevNet rate",
                efficiency
            );
...
async fn execute_optimal_scale_2c(client: &RpcClient, wallet: &Keypair) -> Result<()> {
    ...
    info!(
        "⏰ Step 3: Proven timing optimization ({}ms)...",
        optimization_delay
    );
    ...
    // Step 4: Unwrap
    info!(
        "🔄 Step 4: Unwrapping {} SOL (optimal scale)...",
        amount_sol
    );

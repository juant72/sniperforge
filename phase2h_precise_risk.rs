...
async fn calculate_arbitrage_risk(client: &RpcClient, amount_sol: f64) -> Result<RiskCalculation> {
    ...
    info!(
        ...
        estimated_total_fees as f64 / LAMPORTS_PER_SOL as f64
    );
    info!("       ├── Wrap fee: {} lamports", wrap_fee);
    info!("       ├── Unwrap fee: {} lamports", unwrap_fee);
    info!(
        "       └── Network buffer: {} lamports",
    ...
    info!(
        "     🛡️ Safety buffer (5%): {} lamports ({:.9} SOL)",
        safety_buffer,
    ...
    let minimum_balance_needed =
        amount_lamports + rent_exempt_cost + estimated_total_fees + safety_buffer;
    info!(
        "     🏦 Balance mínimo total: {} lamports ({:.9} SOL)",
        minimum_balance_needed,
        minimum_balance_needed as f64 / LAMPORTS_PER_SOL as f64
    );
    ...
    info!(
        "     📈 Profit esperado ({:.0}%): {} lamports ({:.9} SOL)",
    ...
    info!(
        "     ⚖️ Break-even point: {} lamports ({:.9} SOL)",
        break_even_point,
    ...
    let recommendation = if is_profitable {
        format!(
            "✅ Arbitraje recomendado. Profit esperado: {:.9} SOL, Risk score: {:.1}/10",
            expected_profit as f64 / LAMPORTS_PER_SOL as f64,
            risk_score
        )
    } else if expected_profit <= break_even_point {
        format!(
            "❌ No recomendado. Profit esperado ({:.9} SOL) ≤ break-even ({:.9} SOL)",
            expected_profit as f64 / LAMPORTS_PER_SOL as f64,
            break_even_point as f64 / LAMPORTS_PER_SOL as f64
        )
    } else {
        format!(
            "⚠️ Riesgo alto. Profit margin demasiado bajo: {:.2}x",
            profit_margin
        )
    };
...
fn display_risk_calculation(calc: &RiskCalculation) {
    ...
    info!(
        "   🏦 Balance mínimo requerido: {:.9} SOL",
        calc.minimum_balance_needed as f64 / LAMPORTS_PER_SOL as f64
    );
    info!(
    ...
    info!(
        "   ✅ Es profitable: {}",
        if calc.is_profitable { "SÍ" } else { "NO" }
    );
...
async fn execute_validated_arbitrage(
    ...
    if current_balance < calc.minimum_balance_needed {
        return Err(anyhow::anyhow!(
            "Balance insuficiente en momento de ejecución"
        ));
    }

    let balance_before = current_balance;
    info!(
        "   💳 Balance pre-ejecución: {:.9} SOL",
    ...
    let _wrap_sig =
        execute_validated_wrap(client, wallet, &wsol_account, calc.amount_to_invest).await?;
...
fn analyze_arbitrage_results(
    ...
    if profit > 0.0 {
        let actual_roi =
            (profit / (calc.amount_to_invest as f64 / LAMPORTS_PER_SOL as f64)) * 100.0;

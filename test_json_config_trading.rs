...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if !config.transactions_enabled() {
        ...
        println!(
            "   To enable, set 'enable_real_transactions': true in config/{}.json",
            network
        );
    ...
    let rpc_client =
        RpcClient::new_with_commitment(&config.rpc_endpoint, CommitmentConfig::confirmed());
    ...
    let base_token = config.get_token(&trading_pair.base).ok_or_else(|| {
        format!(
            "Base token '{}' not found in configuration",
            trading_pair.base
        )
    })?;
    let quote_token = config.get_token(&trading_pair.quote).ok_or_else(|| {
        format!(
            "Quote token '{}' not found in configuration",
            trading_pair.quote
        )
    })?;
    ...
    println!(
        "   Base:  {} ({}) - {}",
        base_token.symbol, base_token.name, base_token.address
    );
    println!(
        "   Quote: {} ({}) - {}",
        quote_token.symbol, quote_token.name, quote_token.address
    );
    ...
    if swap_amount > max_allowed {
        println!(
            "❌ Swap amount ({}) exceeds safety limit ({})",
            swap_amount, max_allowed
        );
        return Ok(());
    }
    ...
    println!(
        "   Input: {} {}",
        swap_amount,
        base_token.symbol.to_uppercase()
    );
    ...
    println!(
        "   Output: {} {} tokens",
        quote.out_amount(),
        quote_token.symbol.to_uppercase()
    );
    ...
    if config.transactions_enabled() {
            ...
            .await
        {
            Ok(result) => {
                if result.success {
                    ...
                    println!(
                        "   {}",
                        config.get_explorer_url(&result.transaction_signature)
                    );
            ...
            Err(e) => {
                ...
                if error_str.contains("Route not found") {
                    ...
                    println!(
                        "   • The token pair may not be available on {}",
                        config.display_name
                    );
                    ...
                    println!(
                        "   • Token mint may not be properly initialized on {}",
                        config.display_name
                    );
                    println!(
                        "   • Check program_ids configuration in config/{}.json",
                        network
                    );
                }
            }
        ...
        println!(
            "   To enable real transactions, update config/{}.json:",
            network
        );

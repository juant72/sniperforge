...
impl FastArbitrageHunter {
    async fn new() -> Result<Self> {
        ...
        let wallet_path = std::env::var("SOLANA_WALLET_PATH")
            .unwrap_or_else(|_| "/home/user/.config/solana/mainnet_wallet.json".to_string());
        ...
        Ok(Self {
            ...
            auto_execute_threshold: 0.00015, // 10x fees - ULTRA SAFE
            profit_threshold: 0.000045,      // 3x fees - alert only
            fee_cost: 0.000015,
        })
    }
    ...
    async fn fast_scan_opportunities(&self) -> Result<Vec<OpportunityData>> {
        // Optimized token pairs - focus on most liquid
        ...
        for (pair_name, amount_sol, mint_a, mint_b) in priority_pairs {
            ...
            // Minimal delay - respect rate limits
    ...
    async fn get_jupiter_quote_fast(&self, input_mint: &str, output_mint: &str, amount: u64) -> Result<Option<Value>> {
        ...
        let url = format!(
            "https://quote-api.jup.ag/v6/quote?inputMint={}&outputMint={}&amount={}&slippageBps=100&onlyDirectRoutes=true",

...
async fn test_native_transfer(
    ...
    info!(
        ...
        network_config.program_ids.system_program
    );
    ...
    let transfer_instruction =
        system_instruction::transfer(&sender_keypair.pubkey(), &destination, transfer_amount);
...
async fn test_orca_swap(network_config: &NetworkConfig) -> Result<()> {
    ...
    info!(
        "   Token Program: {}",
        network_config.program_ids.token_program
    );
    info!(
        "   Associated Token Program: {}",
        network_config.program_ids.associated_token_program
    );

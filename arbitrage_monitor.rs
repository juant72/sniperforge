...
impl ArbitrageMonitor {
    ...
    async fn start_monitoring(&self) -> Result<()> {
        ...
        info!("   ⏰ Check interval: {} minutes", self.check_interval_minutes);
        info!("   💰 Profit threshold: {:.9} SOL", self.profit_threshold);
        info!("   📊 Fee cost: {:.9} SOL", self.fee_cost);
        info!("   🎯 Safety margin: {:.1}x fees", self.profit_threshold / self.fee_cost);
        ...
        loop {
            ...
            match self.scan_all_opportunities().await {
                Ok(opportunities) => {
                    if opportunities.is_empty() {
                        ...
                        info!("   🎯 {} PROFITABLE OPPORTUNITIES FOUND!", opportunities.len());
                        ...
                        let best_current = opportunities.iter()
                        ...
                        let excellent_opportunities: Vec<_> = opportunities.iter()
            ...
            // Summary stats
            if cycle_count % 4 == 0 { // Every hour (4 cycles of 15 min)
                info!("\n📊 === HOURLY SUMMARY ===");
                info!("   🔍 Cycles completed: {}", cycle_count);
                info!("   ⏰ Running time: {} hours", (cycle_count * self.check_interval_minutes) / 60);
            ...
            // Wait for next cycle
            info!("   ⏰ Waiting {} minutes until next scan...", self.check_interval_minutes);
    ...
    async fn scan_all_opportunities(&self) -> Result<Vec<OpportunityAlert>> {
        ...
        for (pair_name, amount_sol, mint_a, mint_b) in test_scenarios {
            match self.check_opportunity(mint_a, mint_b, amount_sol, pair_name).await {
                Ok(Some(opportunity)) => {
    ...
    async fn check_opportunity(
        ...
        // Route 1: A → B
        let route_1 = self.get_jupiter_quote(mint_a, mint_b, amount_lamports).await?;
        ...
        if let Some(route_1_data) = route_1 {
            ...
            // Route 2: B → A
            let route_2 = self.get_jupiter_quote(mint_b, mint_a, intermediate_amount).await?;
    ...
    async fn send_alert(&self, opportunity: &OpportunityAlert) {
        ...
        info!("   🛡️  Safety margin: {:.1}x fees", opportunity.profit / self.fee_cost);
    ...
    async fn send_urgent_alert(&self, opportunity: &OpportunityAlert) {
        ...
        info!("   📈 Profit: {:.9} SOL", opportunity.profit);
        info!("   📈 ROI: {:.4}%", opportunity.roi);
        info!("   🛡️  Safety margin: {:.1}x fees (VERY SAFE)", opportunity.profit / self.fee_cost);

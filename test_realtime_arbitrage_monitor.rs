...
impl RealTimeArbitrageMonitor {
                ...
                if response.status().is_success() {
                    ...
                    if let Some(sol_data) = data.get("solana") {
                        if let (Some(price), Some(vol), Some(change)) = (
                            ...
                            self.market_data.insert("SOL".to_string(), RealMarketData {
                            ...
                            info!("   ✅ SOL: ${:.2} (24h: {:.2}%, Vol: ${:.0}M)", 
                                  price, change, vol / 1_000_000.0);
                        }
                    }
                    ...
                    if let Some(usdc_data) = data.get("usd-coin") {
                        if let (Some(price), Some(vol), Some(change)) = (
                            ...
                            self.market_data.insert("USDC".to_string(), RealMarketData {
                    ...
                    if let Some(ray_data) = data.get("raydium") {
                        if let (Some(price), Some(vol), Some(change)) = (
                            ...
                            let sol_price = self.market_data.get("SOL").map(|d| d.price_usd).unwrap_or(157.0);
                            self.market_data.insert("RAY".to_string(), RealMarketData {
    ...
    async fn add_fallback_data(&mut self) {
        ...
        info!("   📊 Precios actualizados: SOL=${:.2}, USDC=${:.4}, RAY=${:.4}", 
    ...
    async fn update_dex_prices(&mut self) {
        ...
        info!("   ✅ Precios de DEX actualizados para {} tokens", self.dex_prices.len());
    }

    async fn detect_arbitrage_opportunities(&self, test_amount: f64) -> Result<Vec<ArbitrageOpportunity>> {
                ...
                if profit_percentage > 0.05 { // At least 0.05% profit
                    ...
                    if net_profit > 0.0 {
                        ...
                        opportunities.push(ArbitrageOpportunity {
                            ...
                            net_profit,
                            dex_route: format!("{} -> {}", lowest_price.dex_name, highest_price.dex_name),
                        ...
                        for ray_price in ray_dex {
                            ...
                            if sol_received > test_amount {
                                ...
                                if profit_percentage > 0.1 { // At least 0.1% profit
                                    ...
                                    if net_profit > 0.0 {
                                        let avg_liquidity = (sol_price.liquidity + usdc_price.liquidity + ray_price.liquidity) / 3.0;
                                        let confidence = self.calculate_confidence_score(
                                            profit_percentage,
                                            avg_liquidity,
                                        ...
                                        opportunities.push(ArbitrageOpportunity {
                                            ...
                                            estimated_gas: (estimated_gas * LAMPORTS_PER_SOL as f64) as u64,
                                            net_profit,
                                            dex_route: format!("{} -> {} -> {}", sol_price.dex_name, usdc_price.dex_name, ray_price.dex_name),
                                            price_impact: profit_sol / test_amount,
                                            execution_time_estimate: 3000 + (rand::random::<f64>() * 2000.0) as u64,
                                        });
                                    }
                                }
                            }
    ...
    fn calculate_confidence_score(&self, profit_percentage: f64, liquidity: f64, price_impact: f64) -> f64 {
        // Confidence based on:
...
async fn main() -> Result<()> {
    ...
    for cycle in 0..30 {
        ...
        if opportunities.is_empty() {
            ...
            for (i, opportunity) in opportunities.iter().enumerate() {
                ...
                info!("     Confidence: {:.1}%", opportunity.confidence_score * 100.0);
                info!("     Gas: {:.4} SOL", opportunity.estimated_gas as f64 / LAMPORTS_PER_SOL as f64);
                info!("     Net Profit: {:.6} SOL", opportunity.net_profit);
                info!("     Execution Time: {}ms", opportunity.execution_time_estimate);
                
                // Execute high-confidence, profitable opportunities
                if opportunity.confidence_score > 0.75 && opportunity.net_profit > 0.0005 {
                    match execute_arbitrage_simulation(
                        &rpc_client,
                        &wallet_keypair,
                        opportunity,
                    ).await {
                    ...
                    info!("     ⏸️ Oportunidad no ejecutada (confidence: {:.1}%, net profit: {:.6})", 
    ...
    if executed_arbitrages > 0 {
        ...
        info!("   🎯 Tasa de éxito: {:.1}%", (executed_arbitrages as f64 / total_opportunities as f64) * 100.0);
    }
    
    // Show final market data
    info!("\n📈 Estado final del mercado:");
    for (symbol, data) in &monitor.market_data {
        info!("   {}: ${:.4} (24h: {:.2}%, Vol: ${:.1}M)", 
              symbol, data.price_usd, data.change_24h, data.volume_24h / 1_000_000.0);
    }
...
async fn execute_arbitrage_simulation(
    ...
    sleep(Duration::from_millis(opportunity.execution_time_estimate / 5)).await;

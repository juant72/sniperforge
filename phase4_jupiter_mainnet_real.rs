    ...
    match execute_jupiter_arbitrage_scan(&user_pubkey).await {
        Ok(opportunity) => {
            if let Some(arb) = opportunity {
                ...
                if arb.expected_profit > 0.000020 { // Más que fees + buffer
...
async fn execute_jupiter_arbitrage_scan(user_pubkey: &Pubkey) -> Result<Option<ArbitrageOpportunity>> {
    ...
    if let Some(route_1_data) = route_1 {
        ...
        if let Some(route_2_data) = route_2 {
            let final_sol_amount: u64 = route_2_data["outAmount"].as_str()
                .unwrap_or("0").parse().unwrap_or(0);
            let final_sol_scaled = final_sol_amount as f64 / LAMPORTS_PER_SOL as f64;
            
            info!("   ✅ USDC → SOL: {} USDC → {} SOL", usdc_amount_scaled, final_sol_scaled);
            ...
            if profit > 0.000015 { // Must beat fees
                ...
                info!("⚠️  Spread too small: {:.9} SOL (< 0.000015 SOL fees)", profit);
            }
        } else {
...
async fn get_jupiter_quote(input_mint: &str, output_mint: &str, amount: u64) -> Result<Option<Value>> {
    ...
    info!("   🌐 Jupiter quote: {} → {} (amount: {})", input_mint, output_mint, amount);
...
async fn execute_real_jupiter_arbitrage(
    ...
    info!("   💰 Expected profit: {:.9} SOL", opportunity.expected_profit);

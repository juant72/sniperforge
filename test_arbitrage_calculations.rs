...
impl ArbitrageTestSystem {
    ...
    async fn test_fee_calculations(&self) -> Result<()> {
        ...
        info!("      🔢 TOTAL FEES: {} lamports ({:.6} SOL)", total_fees, total_fees as f64 / 1_000_000_000.0);
        
        // Verificar que los fees son razonables
        if total_fees > 100000 { // > 0.0001 SOL
    ...
    async fn test_opportunity_detection(&mut self) -> Result<()> {
        ...
        for (input_mint, intermediate_mint, output_mint, route_name) in routes {
            ...
            match self.check_arbitrage_route_test(
                input_mint, 
                intermediate_mint, 
                output_mint, 
                test_amount
                ...
                Some(opportunity) => {
                    ...
                    info!("         💰 Profit bruto: {} lamports", opportunity.profit_lamports);
                    info!("         💸 Total fees: {} lamports", opportunity.all_fees);
                    info!("         💎 Profit neto: {} lamports", opportunity.net_profit);
                    info!("         📈 Porcentaje: {:.4}%", opportunity.profit_percentage);
        ...
        info!("   📊 RESUMEN: {} oportunidades encontradas", opportunities_found);
    ...
    async fn test_slippage_calculations(&self) -> Result<()> {
        ...
        for (amount, token_pair, description) in test_cases {
            ...
            info!("   🔄 {}", description);
            info!("      💰 Amount: {} lamports ({:.6} SOL)", amount, amount as f64 / 1_000_000_000.0);
            info!("      📈 Slippage calculado: {} bps ({:.2}%)", slippage_bps, slippage_bps as f64 / 100.0);
    ...
    async fn test_real_jupiter_quotes(&mut self) -> Result<()> {
        ...
        match self.get_jupiter_quote(SOL_MINT, USDC_MINT, test_amount).await? {
            Some(quote) => {
        ...
        match self.get_jupiter_quote(USDC_MINT, SOL_MINT, usdc_test_amount).await? {
            Some(quote) => {
                let out_amount: u64 = quote["outAmount"]
                    .as_str()
                    .unwrap_or("0")

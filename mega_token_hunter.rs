...
impl MegaTokenHunter {
    ...
    async fn start_mega_hunting(&self) -> Result<()> {
        ...
        info!(
            "   🌟 Scanning {} token pairs with REAL EXECUTION",
            self.get_all_token_pairs().len()
        );
        ...
        loop {
            ...
            match self.mega_scan_all_pairs().await {
                Ok(opportunities) => {
                    ...
                    info!(
                        "   ⚡ MEGA SCAN completed in {:.1}s",
                        scan_duration.as_secs_f64()
                    );

                    if opportunities.is_empty() {
                        info!(
                            "   💤 No opportunities detected across ALL {} pairs",
                            self.get_all_token_pairs().len()
                        );
                    } else {
                        info!(
                            "   🎯 {} opportunities found across mega scan!",
                            opportunities.len()
                        );
                        ...
                        if !auto_exec_opportunities.is_empty() {
                            ...
                            if current_balance < 0.005 {
                                warn!(
                                    "   ⚠️ Balance too low for safe execution: {:.9} SOL",
                                    current_balance
                                );
                                continue;
                            }
                            ...
                            for opp in auto_exec_opportunities.iter().take(3) {
                                ...
                                if opp.profit > self.fee_cost * 0.5 {
                                    info!(
                                        "   🚀 REAL EXECUTION: {} - {:.9} SOL profit ({:.1}x fees)",
                                        opp.pair,
                                        opp.profit,
    ...
    fn get_all_token_pairs(&self) -> Vec<(&str, f64, &str, &str, &str)> {
        vec![
            ...
            (
                ...
                0.001,
                "So11111111111111111111111111111111111111112",
                "BxSuTkn5DQcTBUKyYxjrmBxgL3xL5uUEhz7x2rJ6hS7L",
                "Pepe meme",
            ),
            (
                "SOL/PEPE",
            ...
            (
                ...
                0.001,
                "So11111111111111111111111111111111111111112",
                "mSoLzYCxHdYgdziU2hLnk9NTODiw4PLMpS8kXqGNHhL",
                "Marinade SOL",
            ),
            (
                "SOL/MSOL",
            ...
            (
                ...
                0.001,
                "So11111111111111111111111111111111111111112",
                "7dHbWXmci3dT4YGg48VhMFgkqe7SfGgBDcA5oTfp3gBV",
                "Lido stSOL",
            ),
            (
                "SOL/STSOL",
            ...
            (
                ...
                0.001,
                "So11111111111111111111111111111111111111112",
                "J1tWcv9EShRi6cUxmFbsWLVhzCUyHFvE8QsW5GrSLUhz",
                "Jito SOL",
            ),
            (
                "SOL/JITOSOL",
    ...
    async fn mega_scan_all_pairs(&self) -> Result<Vec<OpportunityData>> {
        ...
        info!(
            "   🔍 Scanning {} token pairs in parallel...",
            token_pairs.len()
        );
        ...
        for chunk in token_pairs.chunks(chunk_size) {
            let futures: Vec<_> = chunk
                .iter()
                .map(|(pair_name, amount, mint_a, mint_b, _desc)| {
    ...
    async fn quick_check_opportunity(
        ...
        if let Ok(Some(route_1_data)) = self
            .get_jupiter_quote_mega(mint_a, mint_b, amount_units)
            .await
        {
            ...
            if intermediate_amount > 0 {
                if let Ok(Some(route_2_data)) = self
                    .get_jupiter_quote_mega(mint_b, mint_a, intermediate_amount)
                    .await
                {
    ...
    async fn get_jupiter_quote_mega(
        &self,
        input_mint: &str,
        output_mint: &str,
        amount: u64,
    ...
    async fn execute_arbitrage(&self, opportunity: &OpportunityData) -> Result<Signature> {
        ...
        info!(
            "   🔄 Step 1: {} -> {}",
            opportunity.mint_a, opportunity.mint_b
        );

    ...
    if !raydium_pools.is_empty() {
        ...
        for (i, pool) in raydium_pools.iter().enumerate() {
            ...
            println!(
                "     Token A: {} ({:.6})",
                pool.token_a,
                pool.reserves_a as f64 / 1_000_000.0
            );
            println!(
                "     Token B: {} ({:.6})",
                pool.token_b,
                pool.reserves_b as f64 / 1_000_000.0
            );
            total_pools += 1;
        }
    }
...
async fn find_real_raydium_pools(client: &RpcClient) -> Result<Vec<RealPool>> {
    ...
    match accounts {
        Ok(accounts) => {
            ...
            for (address, account) in accounts.into_iter().take(5) {
                // Limitar a 5 para test

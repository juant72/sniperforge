...
impl PureDexHunter {
        ...
        loop {
            ...
            match self.scan_pure_dex_opportunities().await {
                Ok(opportunities) => {
                    ...
                    info!(
                        "   ⚡ PURE DEX SCAN completed in {:.1}s",
                    ...
                    if opportunities.is_empty() {
                        ...
                        if !auto_exec_opportunities.is_empty() {
                            info!(
                                "   🔥 {} opportunities above 4x threshold!",
            ...
            if cycle_count % 10 == 0 {
                ...
                info!(
                    "   🎯 Success rate: {:.1}%",
                    (executions as f64 / cycle_count as f64) * 100.0
                );
            }
    ...
    async fn execute_pure_dex_arbitrage(
        &self,
        opportunity: &PureDexOpportunity,
    ) -> Result<Signature> {
        ...
        if swap_amount_lamports < 500 {
            info!(
                "   ⚠️ Amount below minimum: {} lamports",

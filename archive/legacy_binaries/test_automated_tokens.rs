...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Cargar wallet
    let private_key_str =
        env::var("SOLANA_PRIVATE_KEY").expect("SOLANA_PRIVATE_KEY no encontrada en .env");
    ...
    if let (Some(sol_token), Some(test_usdc)) =
        (config.tokens.get("SOL"), config.tokens.get("TEST_USDC"))
    {
        ...
        match jupiter
            .get_quote(&input_mint, &output_mint, amount, None)
            .await
        {
            Ok(quote) => {
    ...
    for (from_symbol, to_symbol) in test_pairs {
        if let (Some(from_token), Some(to_token)) =
            (config.tokens.get(from_symbol), config.tokens.get(to_symbol))
        {
            println!("\n📞 Quote {} -> {}...", from_symbol, to_symbol);

            let jupiter = JupiterApi::new(&config.cluster_url)?;
            let amount = if from_symbol == "SOL" {
                50_000_000
            } else {
                100_000_000
            }; // 0.05 SOL o 100 tokens

            match jupiter
                .get_quote(&from_token.mint, &to_token.mint, amount, None)
                .await
            {
                Ok(quote) => {
                    println!(
                        "✅ Quote disponible: {} -> {} units",

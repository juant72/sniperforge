...
impl VolatilityAwareHunter {
    async fn new() -> Result<Self> {
        ...
        info!(
            "✅ Volatility-aware hunter loaded: {} (from: {})",
    ...
    async fn start_adaptive_hunting(&mut self) -> Result<()> {
        ...
        loop {
            ...
            if previous_mode as u8 != self.current_mode as u8 {
                ...
                info!(
                    "   🎯 New threshold: {:.9} SOL ({:.1}x fees)",
                    self.current_threshold,
                    self.current_threshold / self.fee_cost
                );
            }

            info!(
                "\n🌊 === VOLATILITY CYCLE {} ({:?}) ===",
                cycle_count, self.current_mode
            );
            info!("   📅 Time: {}", now.format("%H:%M:%S"));
            info!(
                "   📊 Volatility: {:.4} | Mode: {:?}",
                self.volatility_score, self.current_mode
            );
                ...
                Ok(opportunities) => {
                    ...
                    if opportunities.is_empty() {
                        ...
                        for opp in sorted_opportunities.iter().take(3) {
                            let status = if opp.profit > self.current_threshold {
                                "🚀 EXECUTE"
                            } else {
                                "💡 MONITOR"
                            };
            ...
            if cycle_count % stats_interval == 0 {
                ...
                info!(
                    ...
                    (cycle_count * self.current_interval) as f64 / 60.0
                );
    ...
    async fn scan_pairs(&self, pairs: &[(&str, f64, &str, &str)]) -> Result<Vec<OpportunityData>> {
        ...
        for (pair_name, amount, mint_a, mint_b) in pairs.iter() {
            if let Ok(Some(opp)) = self
                .quick_check_opportunity(mint_a, mint_b, *amount, pair_name)
                .await
            {
                if opp.profit > 0.0 {
    ...
    async fn quick_check_opportunity(
        ...
        if let Ok(Some(route_1_data)) = self
            .get_jupiter_quote_volatility(mint_a, mint_b, amount_units)
            .await
        {
            ...
            if intermediate_amount > 0 {
                if let Ok(Some(route_2_data)) = self
                    .get_jupiter_quote_volatility(mint_b, mint_a, intermediate_amount)
                    .await
                {
    ...
    async fn get_jupiter_quote_volatility(
        &self,
        input_mint: &str,
        output_mint: &str,
        amount: u64,

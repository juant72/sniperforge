...
impl MilitaryArbitrageSystem {
    ...
    async fn fetch_raydium_pools(&self) -> Result<Vec<PoolData>, Box<dyn std::error::Error>> {
        ...
        for (pubkey, account) in accounts.iter().take(5) {
            // Limitar a 5 para pruebas
            if account.data.len() >= 752 {
                // Tamaño mínimo del pool de Raydium
    ...
    async fn fetch_orca_pools(&self) -> Result<Vec<PoolData>, Box<dyn std::error::Error>> {
        ...
        for (pubkey, account) in accounts.iter().take(5) {
            ...
            if account.data.len() >= 324 {
                // Tamaño mínimo del pool de Orca
    ...
    fn parse_raydium_pool_data(
        &self,
        data: &[u8],
    ) -> Result<(Pubkey, Pubkey, u64, u64, u64), Box<dyn std::error::Error>> {
        if data.len() < 752 {
    ...
    async fn calculate_real_arbitrage_opportunity(
        &self,
        pool_a: &PoolData,
        pool_b: &PoolData,
    ...
    async fn test_real_data_analysis(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for i in 0..valid_pools.len() {
            for j in i + 1..valid_pools.len() {
                ...
                {
                    if let Ok(price_diff) = self
                        .calculate_real_arbitrage_opportunity(pool_a, pool_b)
                        .await
                    {
                        if price_diff > 0.001 {
                            // Mínimo 0.1% diferencia

...
async fn test_multi_dex_arbitrage(
    ...
    for amount in test_amounts {
        ...
        match detector
            .find_arbitrage_opportunities(sol_addr, usdc_addr, amount_lamports, &settings)
            .await
        {
            Ok(opportunities) => {
                if !opportunities.is_empty() {
                    info!(
                        "    ✅ Encontradas {} oportunidades con {} SOL",
                        opportunities.len(),
                        amount
                    );
...
async fn test_cross_token_arbitrage(
    ...
    for (from_name, from_addr, to_name, to_addr) in routes {
        ...
        match detector
            .find_arbitrage_opportunities(from_addr, to_addr, amount_units, &settings)
            .await
        {
            Ok(opportunities) => {
                if !opportunities.is_empty() {
...
async fn test_price_inefficiency_arbitrage(
    detector: &ArbitrageDetector,
    config: &NetworkConfig,
) -> Result<()> {
            ...
            Ok(opportunities) => {
                if !opportunities.is_empty() {
                    for (i, opp) in opportunities.iter().enumerate() {
                        ...
                        if profit_sol > 0.001 {
                            info!(
                                "    🎯 RENTABLE: Orden {} SOL -> Ganancia {:.6} SOL ({:.2}%)",
                                size, profit_sol, profit_percent
                            );
                        } else {
                            info!(
                                "    ⚠️  Orden {} SOL -> Pérdida {:.6} SOL ({:.2}%)",
                                size,
                                profit_sol.abs(),
                                profit_percent.abs()
                            );
...
fn create_program_ids() -> HashMap<String, String> {
    ...
    program_ids.insert(
        "spl_token_swap_program".to_string(),
        "SwaPpA9LAaLfeLi3a68M4DjnLqgtticKg6CnyNwgAC8".to_string(),
    );
    program_ids
}

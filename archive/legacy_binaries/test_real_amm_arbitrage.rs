...
async fn create_custom_swap_pool(
    ...
    let (authority, _bump_seed) =
        Pubkey::find_program_address(&[&pool_pubkey.to_bytes()], &spl_token::ID);

    // Create token accounts for the pool
    let token_a_account =
        spl_associated_token_account::get_associated_token_address(&authority, &mint_a);
    let token_b_account =
        spl_associated_token_account::get_associated_token_address(&authority, &mint_b);
...
async fn detect_arbitrage_opportunities(
    ...
    for start_pool_idx in 0..pools.len() {
        ...
        for second_pool_idx in 0..pools.len() {
            if second_pool_idx == start_pool_idx {
                continue;
            }
            ...
            for third_pool_idx in 0..pools.len() {
                if third_pool_idx == start_pool_idx || third_pool_idx == second_pool_idx {
                    continue;
                }
...
async fn execute_arbitrage_cycle(
    ...
    for (i, &pool_idx) in opportunity.pools.iter().enumerate() {
        ...
        execute_swap_simulation(rpc_client, wallet_keypair, pool, current_amount, step_out).await?;

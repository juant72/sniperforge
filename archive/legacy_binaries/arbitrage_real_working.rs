...
async fn main() -> Result<()> {
    ...
    info!(
        ...
        balance_inicial as f64 / 1_000_000_000.0,
        balance_inicial
    );
    ...
    match jupiter_client
        .execute_swap_with_wallet(&quote_response, &wallet_pubkey.to_string(), Some(&keypair))
        .await
    {
        Ok(result) => {
                ...
                .await
            {
                Ok(result_2) => {
                    ...
                    info!(
                        ...
                        balance_final as f64 / 1_000_000_000.0,
                        balance_final
                    );
                    ...
                    if cambio > 0 {
                        info!(
                            "🎉 ¡ARBITRAJE EXITOSO! +{} lamports (+{:.9} SOL)",
                            cambio,
                            cambio as f64 / 1_000_000_000.0
                        );
                        info!("💰 GANANCIA REAL DEMOSTRADA");
                    } else if cambio < 0 {
                        warn!(
                            "📉 Pérdida: {} lamports ({:.9} SOL)",
                            cambio,
                            cambio as f64 / 1_000_000_000.0
                        );
                    } else {

...
impl EnhancedArbitrageMonitor {
    ...
    async fn update_market_data_with_fallback(&mut self) -> Result<()> {
        ...
        self.performance_tracker.execution_times.push(update_duration);
    ...
    async fn parse_coingecko_data(&mut self, data: serde_json::Value, source: &str) -> Result<()> {
        ...
        if let Some(sol_data) = data.get("solana") {
            if let (Some(price), Some(vol), Some(change), Some(market_cap)) = (
                ...
                self.market_data.insert("SOL".to_string(), EnhancedMarketData {
                ...
                info!("   ✅ SOL: ${:.2} (24h: {:.2}%, Vol: ${:.0}M, σ: {:.2}%)", 
                      price, change, vol / 1_000_000.0, volatility * 100.0);
            }
        }
        ...
        if let Some(usdc_data) = data.get("usd-coin") {
            if let (Some(price), Some(vol), Some(change), Some(market_cap)) = (
                ...
                let sol_price = self.market_data.get("SOL").map(|d| d.price_usd).unwrap_or(157.0);
                let volatility = self.calculate_volatility(change, vol, market_cap);
                self.market_data.insert("USDC".to_string(), EnhancedMarketData {
        ...
        if let Some(ray_data) = data.get("raydium") {
            if let (Some(price), Some(vol), Some(change), Some(market_cap)) = (
                ...
                let sol_price = self.market_data.get("SOL").map(|d| d.price_usd).unwrap_or(157.0);
                let volatility = self.calculate_volatility(change, vol, market_cap);
                self.market_data.insert("RAY".to_string(), EnhancedMarketData {
    ...
    async fn generate_enhanced_fallback_data(&mut self) -> Result<()> {
        ...
        info!("   📊 Datos de respaldo: SOL=${:.2}, USDC=${:.4}, RAY=${:.4}", 
        ...
        for (symbol, market_data) in &self.market_data {
            ...
            for (dex_name, base_spread, liquidity_factor) in &dex_configs {
                ...
                dex_prices.push(DEXPriceData {
                    ...
                    volume_24h: market_data.volume_24h * liquidity_factor * (0.8 + rand::random::<f64>() * 0.4),
                    market_depth,
    ...
    async fn detect_enhanced_arbitrage_opportunities(&mut self, test_amount: f64) -> Result<Vec<ArbitrageOpportunity>> {
        ...
        for (symbol, dex_prices) in &self.dex_prices {
            ...
            if buy_dex.price < sell_dex.price {
                ...
                if profit_percentage > 0.05 && profit_percentage < 5.0 { // Reasonable range
                    let opportunity = self.create_enhanced_opportunity(
                        symbol.clone(),
                        vec![symbol.clone()],
                        buy_dex,
                        sell_dex,
        ...
        self.detect_triangular_arbitrage(&mut opportunities, test_amount, test_amount_lamports).await?;
                ...
                for usdc_price in usdc_dex.iter().filter(|d| d.is_active) {
                    for ray_price in ray_dex.iter().filter(|d| d.is_active) {
                        ...
                        if forward_result.is_profitable {
                            if let Ok(opportunity) = self.create_triangular_opportunity(
                                vec!["SOL".to_string(), "USDC".to_string(), "RAY".to_string(), "SOL".to_string()],
                                test_amount,
                                test_amount_lamports,
                                forward_result,
                                format!("{} -> {} -> {}", sol_price.dex_name, usdc_price.dex_name, ray_price.dex_name),
                        ...
                        if reverse_result.is_profitable {
                            if let Ok(opportunity) = self.create_triangular_opportunity(
                                vec!["SOL".to_string(), "RAY".to_string(), "USDC".to_string(), "SOL".to_string()],
                                test_amount,
                                test_amount_lamports,
                                reverse_result,
                                format!("{} -> {} -> {}", sol_price.dex_name, ray_price.dex_name, usdc_price.dex_name),
    ...
    fn calculate_triangular_result(&self, amount: f64, price1: f64, price2: f64, price3: f64, price4: f64) -> TriangularResult {
    ...
    async fn create_triangular_opportunity(
        ...
        let avg_liquidity = dex_prices.iter().map(|d| d.liquidity).sum::<f64>() / dex_prices.len() as f64;
        let avg_spread = dex_prices.iter().map(|d| d.spread).sum::<f64>() / dex_prices.len() as f64;
        let avg_depth = dex_prices.iter().map(|d| d.market_depth).sum::<f64>() / dex_prices.len() as f64;
    ...
    fn calculate_risk_score(&self, profit_percentage: f64, liquidity: f64, spread: f64, symbol: &str) -> f64 {
    ...
    fn calculate_enhanced_confidence_score(&self, profit_percentage: f64, liquidity: f64, price_impact: f64, risk_score: f64, symbol: &str) -> f64 {
        ...
        let weighted_score = profit_factor * 0.3 + liquidity_factor * 0.25 + impact_factor * 0.2 + risk_factor * 0.15 + symbol_factor * 0.1;
        weighted_score.min(1.0)
    }

    fn calculate_optimal_amount(&self, test_amount: f64, buy_liquidity: f64, sell_liquidity: f64, profit_ratio: f64) -> u64 {
        // Calculate optimal amount based on liquidity and profit ratio
        let max_liquidity = buy_liquidity.min(sell_liquidity);
        let liquidity_constrained = (max_liquidity * 0.05).min(self.risk_parameters.max_position_size); // Max 5% of liquidity
    ...
    fn passes_risk_checks(&self, opportunity: &ArbitrageOpportunity) -> bool {
        ...
        opportunity.confidence_score >= self.risk_parameters.min_confidence
            ...
            && opportunity.estimated_gas as f64 / LAMPORTS_PER_SOL as f64 <= self.risk_parameters.max_gas_cost
...
async fn main() -> Result<()> {
    ...
    let mut monitor = EnhancedArbitrageMonitor::new(
        config.clone(),
        rpc_client.clone(),
    ...
    info!("   Max posición: {:.3} SOL", monitor.risk_parameters.max_position_size);
    info!("   Max slippage: {:.2}%", monitor.risk_parameters.max_slippage * 100.0);
    info!("   Min confianza: {:.0}%", monitor.risk_parameters.min_confidence * 100.0);
    info!("   Max gas: {:.4} SOL", monitor.risk_parameters.max_gas_cost);
    ...
    for cycle in 0..total_cycles {
        ...
        let opportunities = monitor.detect_enhanced_arbitrage_opportunities(test_amount).await?;
        
        if opportunities.is_empty() {
            info!("   ℹ️ No se detectaron oportunidades rentables");
        } else {
            info!("   🎯 {} oportunidades detectadas (filtradas por riesgo):", opportunities.len());
            
            for (i, opportunity) in opportunities.iter().enumerate() {
                info!("   📊 Oportunidad #{} [{}]", i + 1, opportunity.id);
                info!("     Ruta: {:?}", opportunity.path);
                info!("     DEX: {}", opportunity.dex_route);
                info!("     Profit: {:.6} SOL ({:.3}%)", opportunity.profit_sol, opportunity.profit_percentage);
                info!("     Confianza: {:.1}% | Riesgo: {:.1}%", 
                      opportunity.confidence_score * 100.0, opportunity.risk_score * 100.0);
                info!("     Gas: {:.4} SOL | Net: {:.6} SOL", 
                      opportunity.estimated_gas as f64 / LAMPORTS_PER_SOL as f64, opportunity.net_profit);
                info!("     Slippage: {:.2}% | Profundidad: {:.1}%", 
                      opportunity.slippage_tolerance * 100.0, opportunity.market_depth * 100.0);
                ...
                if opportunity.confidence_score > 0.8 && opportunity.net_profit > 0.001 {
                    match execute_enhanced_arbitrage_simulation(opportunity).await {
                        Ok(result) => {
                            ...
                            info!("     💰 Resultado: {:.6} SOL (eficiencia: {:.1}%)", 
                                  result.actual_profit, result.efficiency * 100.0);
                            
                            monitor.metrics.executed_trades.fetch_add(1, Ordering::Relaxed);
                            if result.actual_profit > 0.0 {
                                monitor.metrics.profitable_trades.fetch_add(1, Ordering::Relaxed);
                            }
                        }
                        Err(e) => {
                    ...
                    info!("     ⏸️ Oportunidad no ejecutada (confianza: {:.1}%, net: {:.6})", 
                          opportunity.confidence_score * 100.0, opportunity.net_profit);
                }
            }
        }
        ...
        info!("   ⏱️ Ciclo completado en {:.1}ms", cycle_duration.as_millis());
    ...
    if executed > 0 {
        info!("🎯 Tasa de éxito: {:.1}%", (profitable as f64 / executed as f64) * 100.0);
        info!("📊 Selectividad: {:.1}%", (executed as f64 / total_ops as f64) * 100.0);
    }
    ...
    if monitor.performance_tracker.api_calls > 0 {
        info!("✅ Éxito API: {:.1}%", 
              ((monitor.performance_tracker.api_calls - monitor.performance_tracker.api_errors) as f64 / 
               monitor.performance_tracker.api_calls as f64) * 100.0);
    }
    ...
    for (symbol, data) in &monitor.market_data {
        info!("   {}: ${:.4} (σ: {:.2}%, fuente: {})", 
              symbol, data.price_usd, data.volatility * 100.0, data.data_source);
    }
...
async fn execute_enhanced_arbitrage_simulation(opportunity: &ArbitrageOpportunity) -> Result<ExecutionResult> {
    ...
    let volatility_impact = theoretical_profit * market_volatility * if rand::random::<bool>() { 1.0 } else { -1.0 };
    let slippage_cost = theoretical_profit * actual_slippage;
    let execution_fees = theoretical_profit * 0.001; // 0.1% execution fees
    let gas_premium = (opportunity.estimated_gas as f64 / LAMPORTS_PER_SOL as f64) * network_congestion;
    
    let actual_profit = theoretical_profit + volatility_impact - slippage_cost - execution_fees - (gas_premium * 0.5);
    let efficiency = if theoretical_profit > 0.0 { actual_profit / theoretical_profit } else { 0.0 };
    ...
    info!("         Slippage: -{:.6} SOL ({:.2}%)", slippage_cost, actual_slippage * 100.0);

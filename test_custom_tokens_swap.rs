...
async fn main() -> Result<()> {
    ...
    info!("🚀 === EJECUCIÓN REAL CON TOKENS CUSTOM - DevNet ===");
    info!("===================================================");
    ...
    for (symbol, token_info) in &config.tokens {
        if let Some(supply) = token_info.test_supply {
            info!(
                "   {} ({}): {} (supply: {})",
                symbol, token_info.name, token_info.mint, supply
            );
        } else {
    ...
    if balance_sol < 0.01 {
        error!("❌ Balance insuficiente para trading. Necesitas al menos 0.01 SOL");
        return Ok(());
    }
    ...
    // Execute real swap trades with custom tokens
    info!("\n🎯 === EJECUTANDO SWAPS CON TOKENS CUSTOM ===");

    // Get token addresses
    let sol_mint = config.tokens.get("SOL").unwrap().mint.clone();
    let usdc_mint = config.tokens.get("TEST_USDC").unwrap().mint.clone();
    let ray_mint = config.tokens.get("TEST_RAY").unwrap().mint.clone();

    // Test 1: SOL -> TEST_USDC swap
    info!("\n📊 Test 1: Swap SOL -> TEST_USDC (0.001 SOL)");
    execute_real_swap(
        &jupiter,
        &wallet_keypair,
        &rpc_client,
    ...
    // Test 2: SOL -> TEST_RAY swap
    info!("\n📊 Test 2: Swap SOL -> TEST_RAY (0.001 SOL)");
    execute_real_swap(
        &jupiter,
        &wallet_keypair,
        &rpc_client,
    ...
    info!("\n🎯 === CONCLUSIONES ===");
    info!("✅ Swaps con tokens custom ejecutados exitosamente");
    info!("✅ Transacciones confirmadas en blockchain");
    info!("✅ Sistema listo para arbitraje con tokens reales");
    info!("💡 Próximo paso: Implementar arbitraje automático");
    ...
    let swap_request = sniperforge::shared::jupiter_types::SwapRequest {
        ...
        prioritizationFeeLamports: Some(sniperforge::shared::jupiter_types::PrioritizationFee {
            priorityLevelWithMaxLamports: sniperforge::shared::jupiter_types::PriorityLevelConfig {
                maxLamports: 100000, // 0.0001 SOL max priority fee for devnet

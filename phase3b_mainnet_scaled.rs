    ...
    match execute_scaled_2c_technique(&client, &wallet).await {
        Ok(_) => {
            ...
            if actual_profit > 0.000015 {
                ...
                warn!(
                    "⚠️  Small loss: {:.9} SOL (within expected fee range)",
                    actual_profit
                );
            ...
            info!(
                "   🌐 DevNet 2C:  +{:.9} SOL (ROI: {:.4}%)",
                devnet_profit, devnet_roi
            );
            info!(
                "   🌍 MainNet 3B: {:+.9} SOL (ROI: {:.4}%)",
                actual_profit,
                (actual_profit / 0.01) * 100.0
            );
        }
        Err(e) => {
...
async fn execute_scaled_2c_technique(client: &RpcClient, wallet: &Keypair) -> Result<()> {
    ...
    info!(
        "🔄 Step 2: Wrapping {} SOL (scaled 2C technique)...",
        amount_sol
    );
    ...
    info!(
        "⏰ Step 3: Timing optimization ({}ms, DevNet proven)...",
        optimization_delay
    );

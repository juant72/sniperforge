...
async fn main() -> Result<()> {
    ...
    info!("💰 BALANCE INICIAL: {} SOL ({} lamports)", sol_inicial, balance_inicial);
    ...
    info!("🛒 PASO 1: [SIMULADO] Comprando {} SOL en Orca a ${:.2}", 
          arbitrage_amount as f64 / 1_000_000_000.0, orca_sol_price);
    info!("✅ [SIMULADO] Compra en Orca completada");
    
    info!("💰 PASO 2: [REAL] Vendiendo {} SOL en Jupiter a ${:.2}", 
          arbitrage_amount as f64 / 1_000_000_000.0, jupiter_sol_price);
    ...
    match jupiter_client.execute_swap_with_wallet(
        ...
        Ok(result) => {
            ...
            info!("💰 BALANCE FINAL: {} SOL ({} lamports)", sol_final, balance_final);
            ...
            info!("📊 Cambio:          {:.9} SOL ({} lamports)", cambio_sol, cambio_lamports);
            
            if cambio_sol > 0.0 {
                info!("🎉 ¡ARBITRAJE EXITOSO! +{:.9} SOL", cambio_sol);
                info!("💰 Profit real demostrado: +${:.4}", cambio_sol * jupiter_sol_price);
            } else if cambio_sol < 0.0 {
                warn!("📉 Pérdida en arbitraje: {:.9} SOL", cambio_sol);
                warn!("💸 Pérdida en USD: -${:.4}", (-cambio_sol) * jupiter_sol_price);
            } else {

...
async fn main() -> Result<()> {
    ...
    info!(
        "   💰 Amount: {} SOL (ultra-conservative)",
        ULTRA_CONSERVATIVE_AMOUNT
    );
    ...
    if net_change > 0.0 {
        ...
        warn!(
            "⚠️  Small loss within expected range: {:.9} SOL",
            net_change
        );
...
async fn connect_to_mainnet() -> Result<RpcClient> {
    ...
    for (i, rpc_url) in rpcs.iter().enumerate() {
        let rpc_type = if rpc_url.contains("helius") {
            "Helius (Premium)"
        } else if rpc_url.contains("quicknode") {
            "QuickNode (Premium)"
        } else if rpc_url.contains("alchemy") {
            "Alchemy (Premium)"
        } else if rpc_url.contains("ankr") && rpc_url.contains("api-key") {
            "Ankr (Premium)"
        } else {
            "Public"
        };
...
async fn execute_proven_arbitrage(client: &RpcClient, wallet: &Keypair) -> Result<()> {
    ...
    info!(
        "⏰ Optimization delay: {}ms (2C proven timing)...",
        TIMING_OPTIMIZATION
    );

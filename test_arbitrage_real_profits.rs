...
async fn test_multiple_pairs(detector: &ArbitrageDetector) -> Result<()> {
    ...
    for (from_name, from_addr, to_name, to_addr) in token_pairs {
        ...
        match detector
            .detect_opportunities(from_addr, to_addr, amount)
            .await
        {
            Ok(opportunities) => {
                if opportunities.is_empty() {
                    info!(
                        "    ⚠️  No hay oportunidades para {} -> {}",
                        from_name, to_name
                    );
                } else {
                    info!(
                        "    ✅ Encontradas {} oportunidades para {} -> {}",
                        opportunities.len(),
                        from_name,
                        to_name
                    );
                    ...
                    for (i, opp) in opportunities.iter().enumerate() {
                        if opp.profit_amount > 0.001 {
                            info!(
                                "      🎯 RENTABLE: Oportunidad {}: +{:.6} tokens ({:.2}%)",
                                i + 1,
                                opp.profit_amount,
            ...
            .await
        {
            Ok(opportunities) => {
                for (i, opp) in opportunities.iter().enumerate() {
                    ...
                    if opp.profit_amount > 0.0 {
                        info!(
                            "      🎯 Oportunidad {}: +{:.6} SOL (~${:.2} USD) - {:.2}% ganancia",
                            i + 1,
                            opp.profit_amount,
                            profit_usd,
                            opp.profit_percentage * 100.0
                        ...
                        info!(
                            "      ⚠️  Pérdida: {:.6} SOL (~${:.2} USD) - {:.2}% pérdida",
                            opp.profit_amount.abs(),
                            loss_usd,
                            opp.profit_percentage.abs() * 100.0
                        );
                    }
                }
            }
